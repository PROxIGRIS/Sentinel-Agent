...
============================
    1. SESSION BROKER (boot deadlock fix): the SYSTEM scheduled task now
       runs only a thin supervisor (cmd_host) which spawns the agent into
       the ACTIVE INTERACTIVE SESSION via WTSGetActiveConsoleSessionId +
       WTSQueryUserToken + CreateProcessAsUser, with the user's environment
       block and profile directory exported (OBYLON_USER_PROFILE). The agent
       ...
       respawn, session-flip handling, full handle hygiene.
...
class WorkstationGuard:
    ...
    def _hook_thread_runner(self):
        ...
        # Capture our own thread id so stop() can post WM_QUIT into this
        # message pump from any other thread.
    ...
    def stop(self):
        """Cleanly release input and tear down the hook thread.

        Called by the license-invalidation path in the watchdog. Previously
        missing entirely — WARDEN.stop() would have raised AttributeError
        exactly when the agent was trying to shut down gracefully."""
        with self._lock:
            self.locked = False
            if self.freeze_timer:
                try: self.freeze_timer.cancel()
                except Exception: pass
                self.freeze_timer = None
        tid = getattr(self, "_hook_thread_id", None)
        if tid:
            try:
                user32.PostThreadMessageW(tid, 0x0012, 0, 0)  # WM_QUIT
            except Exception as e:
                logger.error("Warden stop: WM_QUIT post failed", component="warden", error=str(e))
        logger.info("Warden stopped — hooks released", component="warden")
...
def start_wmi_process_monitor():
    """Event-driven process monitor (WMI creation events) with psutil polling
    fallback if WMI itself is unavailable. Both paths share evaluate_new_process."""
    def _wmi_loop():
        try:
            import wmi
            import pythoncom
            ...
            # ---- psutil polling fallback (WMI unavailable) ----
            known_pids = set()
            while True:
                try:
                    current_pids = set()
                    for p in psutil.process_iter(['pid', 'ppid', 'name', 'exe', 'cmdline']):
                        try:
                            ...
                            if pid not in known_pids:
                                evaluate_new_process(p.info, proc_obj=p)
                        except (psutil.NoSuchProcess, psutil.AccessDenied):
                            pass
...
class OcrEngine:
    ...
    def _run(self, image_bytes: bytes) -> float:
        ...
        text = pytesseract.image_to_string(img, timeout=self._timeout) or ""
        if not text.strip():
            return 0.0
...
def main() -> None:
    try:
        ...
        try:
            ...
            pass
def cmd_host():
    """SESSION BROKER — Session 0 supervisor.

    Runs as SYSTEM under the boot-time Scheduled Task (`obylonc boot enable`
    registers `obylon.exe host`). Its ONLY job is to watch the console session
    and (re)spawn the real agent INSIDE the active interactive session.

    Why the old design deadlocked
    -----------------------------
    The boot task previously launched the agent itself as SYSTEM in Session 0.
    Session 0 is a non-interactive isolation window station: no user desktop
    (screenshot/hook/overlay subsystems see nothing), SYSTEM's profile instead
    of the student's (Path.home() -> systemprofile, so the SQLite vault and
    identity files landed in the wrong place), and — the reported killer —
    no per-user network context. School networks routinely gate traffic on
    per-user proxy/WPAD settings and TLS-inspection roots installed only in
    the user's certificate store; a Session 0 SYSTEM process has neither, so
    every Supabase call failed at boot and the agent appeared "dead".

    What this broker does differently
    ---------------------------------
    1. Waits for a real console session (WTSGetActiveConsoleSessionId).
    2. Acquires the SESSION USER's token (WTSQueryUserToken; fallback: borrow
       a token from explorer.exe/sihost.exe living in that session) so the
       agent runs as the student — correct HKCU proxy, correct cert store,
       correct Path.home(), correct desktop heap.
    3. Duplicates it to a PRIMARY token, explicitly pins TokenSessionId,
       builds the user's environment block (CreateEnvironmentBlock) and
       exports OBYLON_USER_PROFILE / OBYLON_SESSION_ID so the agent can
       anchor its files even if token resolution later degrades.
    4. CreateProcessAsUser onto winsta0\\default, hidden window.
    5. Supervises: session flip => terminate old child, spawn in the new
       session; child exit => crash-rate-limited respawn (a tight crash loop
       would otherwise burn CPU and flood the log every 5s forever).
    """
    import win32ts
    ...
    CREATE_NO_WINDOW = getattr(win32con, "CREATE_NO_WINDOW", 0x08000000)
    TOKEN_ALL_ACCESS = getattr(win32con, "TOKEN_ALL_ACCESS", 0xF01FF)

    _PRIVILEGES = (
        "SeTcbPrivilege",                  # WTSQueryUserToken
        "SeAssignPrimaryTokenPrivilege",   # CreateProcessAsUser w/ primary token
        "SeIncreaseQuotaPrivilege",        # CreateProcessAsUser quota handoff
        "SeDebugPrivilege",                # fallback token borrow from shell procs
    )
        ...
        try:
            for name in _PRIVILEGES:
                try:
                    ...
                    logger.warning("privilege unavailable", component="session-broker",
                                   privilege=name, error=str(e))
        finally:
            win32api.CloseHandle(htoken)

    def _query_session_user_token(session_id: int):
        """Return (token_handle, kind) for the interactive user of `session_id`.

        Primary path: WTSQueryUserToken (needs SeTcbPrivilege; we are SYSTEM).
        Fallback: borrow a token from a shell process already running in that
        session (covers the logon race where the session exists but the user
        token isn't queryable yet, and weird kiosks).
        """
        try:
    ...
    def launch_in_active_session(exe_cmdline: str):
        """Spawn the agent in the active console session as the session user.
        Returns (proc_info, session_id) or (None, session_id)."""
        ...
        primary_token = None
        try:
            src_token, token_kind = _query_session_user_token(session_id)
            if src_token is None:
                logger.error("no user token available for session yet",
                             component="session-broker", session_id=session_id)
                return None, session_id

            # Duplicate to a PRIMARY token suitable for process creation.
            ...
            # Pin the token to the target session explicitly — never rely on
            # the token already carrying the right session id.
            try:
                win32security.SetTokenInformation(
                    primary_token, win32security.TokenSessionId, session_id
                )
            except Exception as e:
                logger.warning("TokenSessionId pin failed (continuing)",
                               component="session-broker", error=str(e))

            # User environment block (USERPROFILE/APPDATA/HOMEPATH, per-user
            # proxy env) — the single most important inheritance for the
            # network stack. Fall back to our own env if the hive isn't loaded.
            try:
            ...
            try:
                profile_dir = win32profile.GetUserProfileDirectory(primary_token)
            except Exception:
                profile_dir = None

            env.update({
                "OBYLON_SESSION_ID": str(session_id),
                "OBYLON_SPAWNED_BY": "session-broker",
                "OBYLON_TOKEN_KIND": token_kind or "",
                "SYSTEMROOT": env.get("SYSTEMROOT", os.environ.get("SYSTEMROOT", r"C:\Windows")),
            })
            if profile_dir:
                env["OBYLON_USER_PROFILE"] = profile_dir

            startup = win32process.STARTUPINFO()
            startup.lpDesktop = "winsta0\\default"
            startup.dwFlags = win32con.STARTF_USESHOWWINDOW
            startup.wShowWindow = 0  # SW_HIDE — agent has no console of its own

            proc_info = win32process.CreateProcessAsUser(
                primary_token,
                None,               # application name — use command line
                exe_cmdline,        # full quoted command line
                None, None, False,
                win32con.CREATE_UNICODE_ENVIRONMENT | CREATE_NO_WINDOW,
                env,
                profile_dir or None,   # working directory = user profile
                startup,
            )
            logger.info("agent spawned into interactive session",
                        component="session-broker", session_id=session_id,
                        pid=proc_info[2], token_kind=token_kind)
            return proc_info, session_id
        except Exception as e:
            logger.error("session spawn failed", component="session-broker",
            ...
            if src_token:
                try: win32api.CloseHandle(src_token)
                except Exception: pass
    ...
    if not getattr(sys, 'frozen', False):
        exe_cmdline = f'"{exe_path}" "{os.path.abspath(__file__)}"'
    else:
        exe_cmdline = f'"{exe_path}"'

    child = None               # (hProcess, hThread, pid, tid)
    current_session = None
    spawn_ts = 0.0
    fail_count = 0
    next_spawn_allowed = 0.0

    def _close_child():
        nonlocal child
        if child:
            for handle in (child[0], child[1]):
    ...
    def _terminate_child():
        nonlocal child
        if child:
            try: win32process.TerminateProcess(child[0], 0)
            except Exception: pass
        _close_child()

    logger.info("Session Broker online — waiting for an interactive console session",
                component="session-broker")

    while True:
        try:
            session_id = win32ts.WTSGetActiveConsoleSessionId()
            session_valid = session_id not in (0xFFFFFFFF, None)
            if not session_valid:
                # Nobody logged on yet (typical right after boot). Wait.
                time.sleep(5)
                continue

            if session_id != current_session:
                # Logoff/logon or fast-user-switch: the old child belongs to a
                # dead/detached session — kill it and respawn in the new one.
                if child:
                    logger.info("console session flipped — recycling agent",
                                component="session-broker",
                                old_session=current_session, new_session=session_id)
                _terminate_child()
                current_session = None
                fail_count = 0
                next_spawn_allowed = 0.0
            ...
            if child:
                try:
                    child_alive = (win32process.GetExitCodeProcess(child[0])
                                   == win32con.STILL_ACTIVE)
                except Exception:
                    child_alive = False
                if not child_alive:
                    lifetime = time.time() - spawn_ts
                    exit_code = None
                    try: exit_code = win32process.GetExitCodeProcess(child[0])
                    except Exception: pass
                    _close_child()
                    # A child that dies within 20s counts as a crash, not an exit.
                    if lifetime < 20.0:
                        fail_count += 1
                    else:
                        fail_count = 0
                    backoff = min(15 * max(fail_count, 1), 300)
                    next_spawn_allowed = time.time() + backoff
                    logger.warning("agent child exited — scheduling respawn",
                                   component="session-broker",
                                   exit_code=exit_code, lifetime_sec=int(lifetime),
                                   respawn_in_sec=backoff, consecutive_crashes=fail_count)

            if child is None and time.time() >= next_spawn_allowed:
                proc_info, sid = launch_in_active_session(exe_cmdline)
                if proc_info:
                    child = proc_info
                    current_session = sid
                    spawn_ts = time.time()
                else:
                    # Couldn't spawn (token not ready, etc.) — gentle retry.
                    fail_count += 1
                    next_spawn_allowed = time.time() + min(10 * fail_count, 120)

            time.sleep(4)
        except Exception as e:
            logger.error("broker loop error", component="session-broker",
