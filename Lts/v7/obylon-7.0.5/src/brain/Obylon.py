...
============================
    ...
    7. ADAPTIVE SCAN PRESSURE: local ad-network reputation now contributes
       only to bounded observation pressure. Fresh contextual matches can
       shorten scan cadence and increase OCR/evidence sampling when
       corroborating signals exist; reputation alone never enforces.
"""
...
from recovery_engine import RecoveryClass, RecoveryEngine, RecoveryOutcome
...
def _recovery_log(message: str, **fields):
    try:
...
def _recovery_state_snapshot() -> dict:
    with sys_state.lock:
        return {
            "network": sys_state.network.name,
            "auth": sys_state.auth.name,
            "client": sys_state.client.name,
            "license": sys_state.license.name,
            "sync": sys_state.sync.name,
            "security": sys_state.security.name,
            "session_generation": sys_state.session_generation,
        }


def _recovery_journal_path() -> Path:
    return Path(os.environ.get("PROGRAMDATA", "C:\\ProgramData")) / "Obylon" / "recovery_journal.json"


recovery_engine = RecoveryEngine(
    journal_path=_recovery_journal_path(),
    logger=_recovery_log,
    state_provider=_recovery_state_snapshot,
)
...
class SessionManager:
    ...
    def force_refresh(self) -> bool:
        """Single-flight refresh/rebuild of the authoritative authenticated session."""
        _ensure_session_deps()
        with self._lock:
            sys_state.update_auth(AuthState.REFRESHING, "Forcing active refresh", "session_mgr")

            # Recovery path: a missing client is not a terminal condition. Rebuild
            # it from the already-loaded vault credentials, then continue through
            # the same validation path. This is deliberately bounded to one
            # reconstruction per call; callers own their retry cadence.
            if self._client is None:
                try:
                    ...
                    if self._access_token and not self._is_valid_jwt(self._access_token):
                        self.invalidate_session()
                        sys_state.update_auth(AuthState.AUTH_TERMINAL, "Corrupted access token", "session_mgr")
                        sys_state.update_security(SecurityState.CORRUPT, "Malformed access token", "session_mgr")
                        return False
                    if not self._access_token or not self._refresh_token:
                        sys_state.update_auth(AuthState.AUTH_REQUIRED, "No refreshable session present", "session_mgr")
                        sys_state.update_client(ClientState.ABSENT, "No client and no session", "session_mgr")
                        return False
                    import ssl, urllib.request, urllib.error
                    use_proxy = True
                    try:
                    ...
                    client.auth.set_session(self._access_token, self._refresh_token)
                    self._client = client
                    sys_state.update_client(ClientState.READY, "Client reconstructed", "session_mgr")
                except Exception as e:
                    self._client = None
                    sys_state.update_client(ClientState.FAILED, "Client reconstruction failed", "session_mgr")
                    sys_state.update_auth(AuthState.AUTH_FAILED, str(e), "session_mgr", next_act="bounded retry")
                    logger.error("Client reconstruction failed", component="auth", error=str(e), exc_info=True)
                    return False
            ...
            try:
                ...
                try:
                    ...
                    logger.debug(f"Session readback failed: {get_e}", component="auth")
                    session = None
            ...
            if session:
                ...
                if new_access and not self._is_valid_jwt(new_access):
                    logger.critical("Supabase returned malformed access token; refusing persistence", component="auth")
                    self.invalidate_session()
                    sys_state.update_auth(AuthState.AUTH_TERMINAL, "Malformed refreshed token", "session_mgr")
                    sys_state.update_security(SecurityState.CORRUPT, "Malformed refreshed token", "session_mgr")
                    return False
                if new_access:
                    token_changed = new_access != self._access_token
                ...
                sys_state.update_auth(AuthState.AUTHENTICATED, "Session refreshed/recovered", "session_mgr")
                sys_state.update_client(ClientState.READY, "Authenticated client ready", "session_mgr")
                return True

            sys_state.update_auth(AuthState.AUTH_FAILED, "Session is dead after refresh", "session_mgr", next_act="bounded recovery")
            self.invalidate_session()
            return False
...
import psutil
pyperclip = None  # Lazy-loaded only by DPDP clipboard actions
...
try:
    ...
    sys.exit("Install dependencies: pip install supabase psutil pywin32 structlog")

# Expensive/network/UI dependencies are loaded only when their subsystem is
# actually needed. Importing them while the Core is summoning Brain can make a
# cold Windows boot look dead for minutes, especially on freshly installed
# endpoints under Defender/SmartScreen inspection.
create_client = None
Client = None
ClientOptions = None
httpx = None
...
def _ensure_session_deps():
    global create_client, Client, ClientOptions, httpx
    if create_client is not None:
        return
    from supabase import create_client as _create_client, Client as _Client, ClientOptions as _ClientOptions
...
def verify_server_signature(payload: dict, server_sig: str, hardware_uuid: str | None = None) -> bool:
    try:
        if not server_sig: return False
        if "nacl.signing" not in sys.modules:
            logger.critical("PyNaCl verifier unavailable; refusing to trust signed security state", component="license")
            return False
        ...
        logger.error(f"Signature verify failed: {e}. Payload: {sign_payload.decode()}, Sig: {server_sig}", component="license")
        return False
...
# Optional local ad-network reputation. This is a deterministic contextual
# signal only: it never performs network I/O and never blocks by itself.
AD_NETWORK_REPUTATION = AdNetworkReputationDB(logger=logger)
AD_NETWORK_REPUTATION.load()
...
MACHINE_NODE_NAME_FILE = node_identity.machine_name_path(os.environ.get('PROGRAMDATA', 'C:\\ProgramData'))
...
# warning) instead track repeated hits within a rolling window; crossing a
# tier's threshold promotes THIS alert to the next tier up and it's re-run
# through that tier's normal enforcement.
#
# BUGFIX (2026-09): this ladder must NEVER be able to reach "critical" —
# critical is reserved for content that is independently, on its own,
# critical; repetition of something weaker must never manufacture a
# critical verdict (that's the same failure class as the SessionAccumulator
# fix above, just at the alert-tier level instead of the lexical-score
# level). A previous version of the comment here claimed "high -> critical"
# escalation was part of the design ("Only high -> critical actually
# freezes") — that mapping was never actually present in the dict below
# (there's no "high" entry at all, so repeated "high" alerts don't
# escalate further today), but the comment describing it as intended was
# real evidence someone could "fix" that by adding it back in. The
# assertion below makes that a hard failure instead of a silent policy
# regression: if this dict ever grows an entry whose escalate_to is
# "critical", the agent refuses to start rather than quietly start
# promoting repeated mid-tier alerts into critical enforcement.
# Numbers deliberately differ per tier — tighter/shorter window for
# high-signal tiers, longer for low-signal info/warning.
ESCALATION_LADDER = {
    "info":    {"threshold": 6, "window_sec": 900,  "escalate_to": "warning"},
    "warning": {"threshold": 5, "window_sec": 900,  "escalate_to": "high"},
}
assert all(_rule["escalate_to"] != "critical" for _rule in ESCALATION_LADDER.values()), (
    "ESCALATION_LADDER must never escalate into 'critical' — critical must "
    "only ever be reached by content that is independently critical, never "
    "by repetition of something weaker. See the comment above this dict."
)
    ...
    while time.monotonic() < deadline:
        try:
            ...
            if e.winerror == 231:  # ERROR_PIPE_BUSY
                try:
                    win32pipe.WaitNamedPipe(CORE_PIPE_NAME, 100)
                except Exception:
                    pass
                continue
            time.sleep(0.02)
...
class WorkstationGuard:
    ...
    def lock_workstation(self, duration: int = None, force: bool = False, reason: str = "violation") -> bool:
    ...
    def disengage_freeze(self, reason: str | None = None) -> bool:
...
# Supabase client ownership is centralized in SessionManager.
import threading
TOKEN_ROTATED_EVENT = threading.Event()
REALTIME_CONNECTED = threading.Event()
...
class ObylonVault:
    ...
    def __init__(self, config_file: str = "obylon.enc"):
        ...
        self.last_signature_invalid = False
        self._rejected_data: dict | None = None
    ...
    def load(self) -> bool:
        """Load and integrity-check the encrypted vault exactly once.

        A failed decrypt/parse/signature verification never mutates the live
        state into a fresh deployment. The evidence-bearing file is quarantined
        only for actual load/decrypt/parse/integrity failures.
        """
        with self._lock:
            ...
            try:
                ...
                if data.get("SERVER_SIG"):
                    sig_payload = {
                        "license_id": data.get("LICENSE_ID"),
                        "node_id": data.get("NODE_ID"),
                        "hardware_uuid": data.get("HARDWARE_UUID") or HARDWARE_UUID,
                        "expires_at": (
                            None if data.get("EXPIRES_AT") in (None, "") else data.get("EXPIRES_AT")
                        ),
                        "issued_at": (
                            None if data.get("LAST_HEARTBEAT_OK_AT") in (None, "") else data.get("LAST_HEARTBEAT_OK_AT")
                        ),
                        "status": (
                            None if data.get("LICENSE_STATUS") in (None, "") else data.get("LICENSE_STATUS")
                        ),
                    }
                    sig_hardware_uuid = str(data.get("HARDWARE_UUID") or HARDWARE_UUID or "").strip()
                    if not verify_server_signature(sig_payload, data["SERVER_SIG"], hardware_uuid=sig_hardware_uuid):
                        self.last_signature_invalid = True
                        self._rejected_data = dict(data)
                        logger.critical("Cached vault signature invalid — possible tampering or stale pre-repair state", component="vault")
                        raise ValueError("cached vault signature invalid")
                ...
                logger.error(
                    f"Config corruption detected (vault preserved for investigation): {e}",
                    component="vault",
                    exc_info=True,
                )
                try:
                    ...
                    if os.path.exists(corrupt_path):
                        corrupt_path = f"{self.config_file}.corrupt.{int(time.time())}"
                    os.replace(self.config_file, corrupt_path)
                    logger.critical(
                        "Vault quarantined after integrity/decryption failure.",
                        component="vault",
                        path=corrupt_path,
                    )
                except Exception as quarantine_err:
                    logger.error(
                        f"Failed to quarantine corrupt vault: {quarantine_err}",
                        component="vault",
                        exc_info=True,
                    )
                self._data = {}
    ...
    def repair_signed_state_from_server(self) -> bool:
        """Repair a stale/invalid cached signature only through the pinned enrollment endpoint.

        The rejected vault is never trusted as authoritative state. Its retained session token is
        used only to ask the server for a fresh, hardware-bound, cryptographically signed state.
        Continuity is required for license_id and node_id before the repaired vault is persisted.
        """
        candidate = dict(self._rejected_data or {})
        access_token = str(candidate.get("ACCESS_TOKEN") or "").strip()
        if not access_token:
            return False
        current_hw = str(HARDWARE_UUID or "").strip()
        if not current_hw:
            return False
        try:
            req = urllib.request.Request(
                f"{ENROLLMENT_ENDPOINT}/license_heartbeat",
                data=json.dumps({"hardware_uuid": current_hw}).encode("utf-8"),
                headers={"Authorization": f"Bearer {access_token}", "Content-Type": "application/json"},
            )
            import ssl, certifi
            ctx = ssl.create_default_context(cafile=certifi.where())
            response = _robust_urlopen(req, timeout=5, component_name="vault")
            with response:
                data = json.loads(response.read().decode("utf-8"))
            server_sig = data.get("server_sig")
            if not server_sig or not verify_server_signature(data, server_sig, hardware_uuid=current_hw):
                logger.error("Signed vault repair rejected: fresh server signature invalid", component="vault")
                return False
            candidate_license = str(candidate.get("LICENSE_ID") or "")
            candidate_node = str(candidate.get("NODE_ID") or "")
            if candidate_license and str(data.get("license_id") or "") != candidate_license:
                logger.error("Signed vault repair rejected: license continuity mismatch", component="vault")
                return False
            if candidate_node and str(data.get("node_id") or "") != candidate_node:
                logger.error("Signed vault repair rejected: node continuity mismatch", component="vault")
                return False

            repaired = dict(candidate)
            repaired["HARDWARE_UUID"] = current_hw
            repaired["LICENSE_ID"] = data.get("license_id") or candidate.get("LICENSE_ID")
            repaired["NODE_ID"] = data.get("node_id") or candidate.get("NODE_ID")
            repaired["LICENSE_STATUS"] = data.get("status") or candidate.get("LICENSE_STATUS")
            repaired["LAST_HEARTBEAT_OK_AT"] = data.get("issued_at") or candidate.get("LAST_HEARTBEAT_OK_AT")
            repaired["MAX_SEEN_UTC"] = max(str(candidate.get("MAX_SEEN_UTC") or ""), str(data.get("issued_at") or "")) or None
            repaired["EXPIRES_AT"] = data.get("expires_at") or candidate.get("EXPIRES_AT")
            repaired["GRACE_DAYS"] = data.get("grace_days", candidate.get("GRACE_DAYS", 14))
            repaired["SERVER_SIG"] = server_sig
            if data.get("access_token"):
                repaired["ACCESS_TOKEN"] = data.get("access_token")
            if data.get("refresh_token"):
                repaired["REFRESH_TOKEN"] = data.get("refresh_token")
            ...
            self.last_load_error = None
            self.last_signature_invalid = False
            logger.info("Cached vault repaired from fresh signed server state", component="vault")
            return True
        except Exception as e:
            logger.error("Signed vault repair failed", component="vault", error=str(e), exc_info=True)
            return False

    def _unhide_file(self):
        """Forcefully strips the hidden/readonly attributes so Python can write."""
        self._unhide_file_for_path(self.config_file)

    def _unhide_file_for_path(self, path: str):
        """Best-effort removal of hidden/readonly attributes for one path."""
    ...
    def _save(self):
        """Atomically persist the encrypted vault.

        The plaintext JSON snapshot is serialized while holding the vault
        lock. The encrypted bytes are written to a temporary file, flushed and
        fsynced, then atomically replaced into place. The fallback path uses the
        same procedure.
        """
        with self._lock:
            json_str = json.dumps(self._data, ensure_ascii=False, separators=(",", ":"))
            encrypted = self._encrypt(json_str.encode("utf-8"))

            primary_path = self.config_file
            fallback = os.path.join(str(USER_HOME), ".obylon_vault_fallback.enc")

            def atomic_write(target: str) -> None:
                target_dir = os.path.dirname(os.path.abspath(target)) or "."
                os.makedirs(target_dir, exist_ok=True)
                self._unhide_file_for_path(target)
                ...
                try:
                    ...
                    try:
                        import ctypes
                        ctypes.windll.kernel32.SetFileAttributesW(str(target), 2)
                    except Exception:
                        pass
                except Exception:
                    try:
            ...
            try:
                atomic_write(primary_path)
                return
            except PermissionError:
                logger.warning(
                    "VAULT WRITE DENIED on primary path — attempting user-profile fallback.",
                    component="vault",
            ...
            try:
                ...
                logger.error(
                    f"Fallback vault write failed: {e}",
                    component="vault",
                    exc_info=True,
                )
                raise
            ...
            if self.config_file != fallback:
                logger.warning(
                    "Vault switched permanently to user-profile fallback for this process.",
                    component="vault",
                    primary=primary_path,
                    fallback=fallback,
                )
    ...
    def provision_via_license(self, license_key: str, hostname: str, hardware_uuid: str, hardware_fingerprint: str) -> str:
        try:
            safe_node_name = (hostname or self._data.get("WORKSTATION_NAME") or socket.gethostname() or "OBYLON-ENDPOINT").strip() or "OBYLON-ENDPOINT"
            payload = {
                "license_key": license_key,
                "hostname": safe_node_name,
                "node_name": safe_node_name,
                "nodeName": safe_node_name,
                "device_name": safe_node_name,
                "deviceName": safe_node_name,
                "hardware_uuid": hardware_uuid,
            ...
            response = _robust_urlopen(req, timeout=30.0, component_name="vault")
            ...
            with response:
                ...
                if "server_sig" in data and not verify_server_signature(data, data["server_sig"], hardware_uuid=hardware_uuid):
                ...
                self._data = {
                    ...
                    "LICENSE_ID": data.get("license_id"),
                    "NODE_ID": data.get("node_id"),
                    "NODE_NAME": data.get("node_name") or data.get("nodeName") or safe_node_name,
                    "WORKSTATION_NAME": safe_node_name,
...
def _normalized_node_name(value: object, fallback: str = "OBYLON-ENDPOINT") -> str:
    value = str(value or "").strip()
    if not value:
        value = str(fallback or "").strip() or "OBYLON-ENDPOINT"
    return value[:128]


def get_workstation_identity(persisted_name: str | None = None) -> str:
    """Resolve the machine-wide display name, then legacy/user fallbacks.

    A node name is a label, never an identity key. The machine-wide label is
    stored under ProgramData so shared-school PCs do not acquire one name per
    student profile. Legacy per-user aliases remain supported for migration.
    """
    try:
        machine_name = node_identity.load_machine_name(os.environ.get("PROGRAMDATA", "C:\\ProgramData"))
        if machine_name:
            return _normalized_node_name(machine_name)
    except Exception as e:
        logger.warning("machine node-name read failed", component="identity", error=str(e))
    try:
        if ALIAS_FILE.exists():
            alias = _normalized_node_name(ALIAS_FILE.read_text(encoding="utf-8"), "")
            if alias and alias != "OBYLON-ENDPOINT":
                return alias
    except Exception as e:
        logger.error("alias read failed", component="identity", error=str(e), exc_info=True)
    persisted = str(persisted_name or "").strip()
    if persisted:
        return _normalized_node_name(persisted)
    return _normalized_node_name(socket.gethostname())
...
SCAN_INTERVAL = 1
SCAN_PRESSURE_MIN_INTERVAL = 0.35
ACTION_POLL = 1
KEYLOG_DURATION = 10
...
COMMAND_TTL_SEC = 60
# A remote shutdown must be a control-plane operation, not a 10-second grace
# sleep followed by a shell call. Evidence capture is best-effort and runs
# independently while the shutdown request is sent immediately.
FINGERPRINT_BOOT_MAX_ATTEMPTS = 6
FINGERPRINT_BOOT_RETRY_DELAY_SEC = 2
# BUGFIX (2026-09): must stay ABOVE the Go helper's internal WMI/CIM deadline
# (obylonc/internal/platform/windows.go: fingerprintTimeout = 12s), or
# subprocess.run() kills `obylonc internal-fingerprint` mid-query on every
# cold boot before WMI has answered — the old value here was 5.0s, well
# under Go's 12s, so the first several boot-time probes were guaranteed to
# be preempted (logged as "hardware fingerprint helper timed out"/"failed")
# regardless of how patient the retry loop below was willing to be.
FINGERPRINT_HELPER_TIMEOUT_SEC = 15.0

# Local single-process action lease. Realtime CDC and the 10s HTTP fallback
# intentionally consume the same pending rows. A database UPDATE followed by
# SELECT is not a reliable claim primitive under all Supabase/RLS policies, so
# the two consumers first arbitrate locally, then write the final DB status
# only after the command was actually dispatched. Successful actions are held
# in a short completed cache so a failed status update cannot execute the same
# command twice. The Broker's duplicate-process prevention remains the outer
# process-level guard.
_ACTION_CLAIM_LOCK = threading.RLock()
_ACTION_CLAIM_LEASE_SEC = 30.0
_ACTION_CLAIMED: dict[str, float] = {}
_ACTION_COMPLETED: dict[str, float] = {}

def _claim_admin_action(action_id: str) -> bool:
    if not action_id:
        return False
    now = time.monotonic()
    with _ACTION_CLAIM_LOCK:
        expired_completed = [k for k, deadline in _ACTION_COMPLETED.items() if deadline <= now]
        for key in expired_completed:
            _ACTION_COMPLETED.pop(key, None)
        expired_claims = [k for k, claimed_at in _ACTION_CLAIMED.items() if now - claimed_at > _ACTION_CLAIM_LEASE_SEC]
        for key in expired_claims:
            _ACTION_CLAIMED.pop(key, None)
        if action_id in _ACTION_CLAIMED or action_id in _ACTION_COMPLETED:
            return False
        _ACTION_CLAIMED[action_id] = now
        return True

def _finish_admin_action(action_id: str, success: bool) -> None:
    if not action_id:
        return
    with _ACTION_CLAIM_LOCK:
        _ACTION_CLAIMED.pop(action_id, None)
        if success:
            _ACTION_COMPLETED[action_id] = time.monotonic() + COMMAND_TTL_SEC
...
# ---------- Fast-lane critical keywords (2026-09) ----------
# ObylonCore.exe's fast lane (see FAST LANE section in core/src/main.rs) is
# a blind, un-contextualized, unanchored substring match against the
# foreground window title — no scoring, no OCR, no Angel Engine, no FSM,
# just `title.contains(keyword)` -> instant freeze + screenshot + report.
# That means every entry here needs to be TRUE 100% OF THE TIME with ZERO
# legitimate counter-example, because nothing downstream gets a chance to
# reconsider it. That bar rules out most of LEXICON["critical"] above,
# even though it's tagged the same severity:
#
#   - Site/brand names and unambiguous explicit slang (pornhub, xvideos,
#     onlyfans, childporn, ...) clear the bar: no legitimate window title
#     will ever contain them as a substring.
#   - Plain dictionary words that ALSO happen to be adult-content terms
#     (sex, nude, naked, adult, dating, escort, erotic) do NOT clear the
#     bar — "sex" alone is a substring of "Middlesex", "Essex", "Sussex",
#     "Wessex"; "dating" is a substring of "Carbon Dating Lab Report" and
#     "Who Is He Dating - IMDb"; "adult" is a substring of "Young Adult
#     Fiction Reading List". These stay Python-only, where LexEngine +
#     the Angel Engine can actually weigh context before anything fires.
#   - Violence/self-harm/terrorism/drug/weapon terms (gore, murder,
#     suicide, terrorist, jihad, bomb, execution, meth, fentanyl, ...) are
#     deliberately excluded even though several are tagged "critical" in
#     LEXICON above. These are genuinely contextual — "suicide prevention
#     week", "the assassination of Julius Caesar", "WWII bomber aircraft
#     history" are all real, legitimate window titles — and Python's Angel
#     Engine (ANGEL_MARKERS_BY_CATEGORY / ANGEL_AGGRAVATORS_BY_CATEGORY)
#     already exists specifically to tell those apart from "how to make a
#     pipe bomb" or "painless suicide methods". A blind Rust substring
#     match has no way to make that call, so it doesn't get to try. This
#     mirrors the code's own existing distinction: the Angel Engine's
#     "adult_explicit" category is the ONLY category with an empty,
#     never-defused marker set (see ANGEL_MARKERS_BY_CATEGORY above) —
#     every other category (self_harm, explosives, weapons, drugs) has
#     real defusal markers because it's genuinely contextual. This list is
#     exactly "adult_explicit", nothing more.
#
# Curated from LEXICON["critical"], _ANGEL_CATEGORY_KEYWORDS["adult_explicit"],
# and MULTILINGUAL_REGISTRY's "critical"-tagged, non-suicide entries.
# Synced into ObylonCore.exe's fast lane by write_fastlane_rules() below —
# see FastLaneRules/default_fastlane_rules() in core/src/main.rs for the
# compiled-in floor Core falls back to before this ever syncs.
FASTLANE_CRITICAL_KEYWORDS: tuple[str, ...] = (
    # -- Known adult-site brand/domain names (no benign meaning, ever) --
    "pornhub", "xvideos", "xnxx", "xnx", "redtube", "brazzers", "hentai",
    "rule34", "xxx", "nsfw", "gelbooru", "onlyfans", "chaturbate",
    "xhamster", "spankbang", "eporner", "camgirl", "nhentai", "youporn",
    "bdsmtube", "camsoda", "stripchat", "myfreecams", "manyvids",
    "fansly", "loyalfans", "hentai haven", "hanime", "jav", "doujinshi",
    "ero-manga",
    # -- Unambiguous explicit acts/slang (English) --
    "p0rn", "pr0n", "p0rnhub", "xvid3os", "sexcam", "jerkoff",
    "masturbate", "blowjob", "handjob", "cumshot", "creampie", "bukkake",
    "bukakke", "gangbang", "orgy", "incest", "stepmom", "stepbro",
    "stepdad", "milf", "gilf", "teenporn", "childporn", "lolita", "shota",
    "guro", "snufffilm", "deepfake porn", "fappening", "nude leak",
    # -- Regional (Hindi/desi) explicit slang --
    "chudai", "savita bhabhi", "kavita bhabhi", "desi mms", "mallu aunty",
    "bihari bhabhi", "mms leak", "bhojpuri bhabhi", "desi chudai",
    "bhabhi sex", "suhagrat", "nangi photo", "kamukata", "choda chudi",
    "choti golpo", "antarvasna", "desipapa", "sexkahani", "chudasi",
    # -- Foreign-language explicit terms (Latin script) --
    "puta", "putaria", "porno", "sexo", "follar", "salope",
    "pornographie", "ficken",
    # -- Native-script explicit terms (Hindi/Arabic/Russian/Chinese/Korean) --
    "पोर्न", "सेक्स", "चुदाई",
    "سكس", "إباحية", "اباحية", "قحبة", "عاهرة", "نيك",
    "порно", "порн", "порнуха",
    "色情", "黄色网站", "成人网站", "裸聊",
    "야동", "섹스", "포률노",
)
...
def _load_good_vocab() -> list[str]:
    ...
    candidates = [
        os.path.join(repo_root, "assets", "data", "good_vocab.txt"),
    ]
    if hasattr(sys, '_MEIPASS'):
        candidates.insert(0, os.path.join(sys._MEIPASS, "assets", "data", "good_vocab.txt"))
        ...
        try:
            ...
            while True:
                try:
                    ...
                    try:
                        # BUGFIX (2026-09): this read "WORKSTATION_ID", a
                        # vault key that is never written anywhere in this
                        # file — only "NODE_ID" is. `if wid:` was therefore
                        # always False and this alert path (and three
                        # others below using the same wrong key) never
                        # actually fired, silently, since whenever this
                        # code was written.
                        wid = vault.get("NODE_ID")
                        if wid:
...
def _handle_core_event(evt: dict) -> None:
    ...
    if kind == "new_network_adapter":
        ...
        severity = "critical" if suspect else "warning"
    ...
    def score(self, title: str, proc: str) -> tuple[float, str, str]:
        ...
        with _perf_section("scanner", "lexical"):
            ...
            for token in tokens:
                ...
                # BUGFIX (2026-09): a run of merely-borderline (0.35-0.70,
                # i.e. NOT independently critical) hits used to be able to
                # multiply its way straight past CRITICAL_SEVERITY_FLOOR
                # (0.85) purely by repeating — 3 weak hits in 60s could
                # yield up to a 1.6x multiplier, turning e.g. 0.70 into 1.12
                # and getting the full critical-tier freeze/webcam/Telegram
                # response even though no single hit was ever strong enough
                # to independently earn it. That's a frequency-based
                # promotion to critical, exactly the pattern this system
                # must never produce — critical means critical, not "this
                # happened enough times." The accumulator's actual, still-
                # legitimate job is surfacing an evasion-probing PATTERN by
                # escalating the response into warning/high territory; it
                # must never be the thing that manufactures a "critical"
                # verdict out of content that was never independently that
                # severe. So: the multiplier still applies in full when the
                # base hit was ALREADY at/above the critical floor on its
                # own (reinforcing a genuinely critical read, still capped
                # at 0.99) — but when the base hit was sub-critical, the
                # boosted score is hard-capped just under the critical
                # floor, however many times it has repeated.
                session_mult = _SESSION_ACC.record(c_lev, bad_term)
                boosted = c_lev * session_mult
                if c_lev < CRITICAL_SEVERITY_FLOOR:
                    boosted = min(boosted, CRITICAL_SEVERITY_FLOOR - 0.01)
                c_lev = min(boosted, 0.99)
...
def _sanitize_unit_score(raw: object) -> float:
        """Normalize untrusted extension scores to a finite [0,1] value."""
        try:
            value = float(raw)
...
def _reputation_adjusted_monetization_score(hostname: str | None, extension_score: object) -> tuple[float, dict]:
        """Fuse extension telemetry with the local reputation DB conservatively.

        The DB contributes at most a contextual floor when a domain is explicitly
        known. It is intentionally not an enforcement verdict. Existing extension
        telemetry remains authoritative when it is stronger.
        """
        ext_score = max(0.0, min(1.0, _sanitize_unit_score(extension_score)))
        match = AD_NETWORK_REPUTATION.lookup(hostname)
        if not match:
            return ext_score, {"matched": False, "contextual_score": 0.0}

        reputation_score = match.contextual_score
        # Mixed-use networks are down-weighted again to reduce false positives.
        if match.mixed_usage:
            reputation_score *= 0.65
        combined = max(ext_score, min(1.0, reputation_score))
        return combined, {
            "matched": True,
            "network_id": match.network_id,
            "category": match.category,
            "contextual_score": round(reputation_score, 4),
        }
...
# NOTE: _background_keylogger() and the Python keyboard listener listener it ran are gone —
        ...
        async for message in websocket:
            try:
                ...
                with _OPTICS_LOCK:
                    ...
                    _LATEST_TRIPWIRE_SCORE = max(0.0, min(1.0, _sanitize_unit_score(data.get("tripwire_score", 0.0))))
                    _LATEST_MONETIZATION_SCORE, reputation_meta = _reputation_adjusted_monetization_score(
                        _LATEST_URL_HOSTNAME or data.get("url", ""), data.get("monetization_score", 0.0)
                    )
                    _LATEST_ALE_SCORE = max(0.0, min(1.0, _sanitize_unit_score(data.get("ale_score", 0.0))))

                    if reputation_meta.get("matched"):
                        logger.info("Ad-network reputation matched", component="optics", **reputation_meta)
...
_HARDWARE_UUID_GENERATED_THIS_BOOT = False

def load_or_create_hardware_uuid() -> str:
    global _HARDWARE_UUID_GENERATED_THIS_BOOT
    try:
    ...
    _HARDWARE_UUID_GENERATED_THIS_BOOT = True
    try:
        IDENTITY_FILE.parent.mkdir(parents=True, exist_ok=True)
        IDENTITY_FILE.write_text(new_id, encoding="utf-8")
        _hide_path(IDENTITY_FILE)
        logger.info("minted provisional machine identity", component="identity", identity_file=str(IDENTITY_FILE))
    except Exception as e:
...
def get_hardware_fingerprint(timeout: float = FINGERPRINT_HELPER_TIMEOUT_SEC) -> str | None:
...
def get_hardware_fingerprint_blocking(timeout: float = 0.0) -> str | None:
    """Read the most recent live fingerprint captured by _identity_verification_loop.

    BUGFIX (2026-09): this used to read a cache that was only ever populated
    by `_compute_hardware_fingerprint_async`, a one-shot background probe
    that nothing actually started (the `.start()` call described in this
    file's own comments never made it back in when identity verification was
    refactored into `_identity_verification_loop`). So `_hardware_fingerprint_ready`
    was never set, and every caller here — including `register_workstation()`
    for brand-new machines, and the recovery engine's `recover_identity`
    action — blocked for its full `timeout` and then unconditionally got
    `None`. `_identity_verification_loop` now populates this cache itself
    (it's the one real probe loop that runs), so this keeps working as
    documented without a second, redundant WMI/CIM query in flight.
    """
    if timeout > 0:
        _hardware_fingerprint_ready.wait(timeout=timeout)
    elif not _hardware_fingerprint_ready.is_set():
        return None
    return _hardware_fingerprint_result.get("value")


def get_verified_hardware_fingerprint(
    max_attempts: int = FINGERPRINT_BOOT_MAX_ATTEMPTS,
    retry_delay_sec: float = FINGERPRINT_BOOT_RETRY_DELAY_SEC,
) -> str | None:
    ...
    for attempt in range(1, attempts + 1):
        fp = get_hardware_fingerprint(timeout=FINGERPRINT_HELPER_TIMEOUT_SEC)
        if _is_hardware_fingerprint(fp):
            return fp
        if attempt < attempts:
...
def _promote_provisional_hardware_uuid(live_fingerprint: str | None) -> None:
    """Replace only a freshly-created provisional id with a deterministic one.

    Existing installations never change hardware_uuid, preserving compatibility
    with licenses, telemetry and historical workstation rows. Fresh installs
    become reinstall-stable when the verified hardware fingerprint is available.
    """
    global HARDWARE_UUID, _HARDWARE_UUID_GENERATED_THIS_BOOT
    if not _HARDWARE_UUID_GENERATED_THIS_BOOT:
        return
    deterministic = node_identity.deterministic_hardware_uuid(live_fingerprint)
    if not deterministic:
        return
    try:
        IDENTITY_FILE.parent.mkdir(parents=True, exist_ok=True)
        IDENTITY_FILE.write_text(deterministic, encoding="utf-8")
        _hide_path(IDENTITY_FILE)
        HARDWARE_UUID = deterministic
        logger.info("promoted provisional identity to hardware-bound identity", component="identity")
    except Exception as e:
        logger.warning("could not persist hardware-bound identity; retaining provisional id", component="identity", error=str(e))
...
def validate_identity_integrity(vault, live_fingerprint: str | None) -> bool:
    ...
    if not stored_fp:
        sys_state.update_security(
            SecurityState.LEGACY_UNVERIFIED,
            "Legacy/no activation fingerprint",
            "identity",
            next_act="activation fingerprint required for verified identity",
        )
        logger.warning(
            "No activation fingerprint present; identity is LEGACY/UNVERIFIED and cannot satisfy security-ready.",
            component="identity",
        )
        return True
    if not _is_hardware_fingerprint(stored_fp):
        sys_state.update_security(SecurityState.CORRUPT, "Stored activation fingerprint is invalid", "identity")
        logger.critical("Stored activation fingerprint is corrupt", component="identity")
        return False
    if not _is_hardware_fingerprint(live_fingerprint):
        sys_state.update_security(SecurityState.UNKNOWN, "Live hardware fingerprint unavailable", "identity", next_act="bounded retry")
        logger.warning("Hardware fingerprint unavailable during boot; security readiness withheld", component="identity")
        return False
    if stored_fp.lower() != live_fingerprint.lower():
        sys_state.update_security(SecurityState.IDENTITY_MISMATCH, "Hardware fingerprint mismatch", "identity")
        logger.critical(
        ...
        return False
    sys_state.update_security(SecurityState.READY, "Hardware identity verified", "identity")
...
def _stable_node_id(hardware_uuid: str, license_id: str | None) -> str:
    """Deterministic UUID for a machine+license pair when no server id exists."""
    import uuid as _uuid
    material = f"obylon-node:{str(license_id or '').strip()}:{str(hardware_uuid).strip().lower()}"
    return str(_uuid.uuid5(_uuid.NAMESPACE_URL, material))


def _server_rows_by_hardware_fingerprint(client, fingerprint: str | None) -> tuple[list[dict], bool]:
    """Find nodes by hardware binding without making the new DB column mandatory.
    ...
    if not fingerprint:
        return [], False
    try:
        result = (client.table("workstations")
                  .select("id,name,hardware_uuid,hardware_fingerprint")
                  .eq("hardware_fingerprint", fingerprint)
                  .execute())
        direct_matches = result.data or []
        if direct_matches:
            return direct_matches, True
        # A migrated backend may have the new column while older rows still
        # have NULL there. Fall through to the existing JSON column so those
        # rows can be recovered without requiring a dangerous bulk backfill.
    except Exception:
        pass
    try:
        result = (client.table("workstations")
                  .select("id,name,hardware_uuid,os_info")
                  .execute())
    except Exception:
        return [], False
    matches = []
    for row in (result.data or []):
        info = row.get("os_info") if isinstance(row, dict) else None
        binding = info.get("obylon_identity") if isinstance(info, dict) else None
        if isinstance(binding, dict) and node_identity.normalize_fingerprint(binding.get("hardware_fingerprint")) == fingerprint:
            matches.append(row)
    return matches, False


def _identity_os_info(fingerprint: str | None) -> dict:
    info = os_info()
    normalized = node_identity.normalize_fingerprint(fingerprint)
    if normalized:
        info["obylon_identity"] = {
            "schema_version": node_identity.SCHEMA_VERSION,
            "hardware_fingerprint": normalized,
            "binding_digest": node_identity.server_identity_digest(normalized),
        }
    return info


def register_workstation() -> str:
    """Reconcile this endpoint to one canonical workstation row.

    Identity precedence is deliberately strict:
      1. persisted server node_id;
      2. persisted machine hardware_uuid;
      3. verified hardware fingerprint (including current-schema os_info fallback);
      4. deterministic hardware-bound UUID for genuinely new machines.

    Hostnames and display names are never identity keys.
    """
    global NODE_ID
    with _NODE_REGISTRATION_LOCK:
        ...
        while attempt < max_retries:
            ...
            try:
                ...
                vault_node_id = str(vault.get("NODE_ID") or NODE_ID or "").strip()

                # For an already-activated workstation, the persisted NODE_ID
                # is the authoritative control-plane key. Do not re-run the
                # PowerShell/CIM fingerprint helper just to refresh an existing
                # row. Fingerprint acquisition remains necessary for a genuine
                # new/relinked machine, where it is done below without blocking
                # the main boot path.
                live_fingerprint = None
                if not vault_node_id:
                    try:
                        live_fingerprint = get_hardware_fingerprint_blocking(timeout=5.0)
                    except Exception:
                        live_fingerprint = None
                _promote_provisional_hardware_uuid(live_fingerprint)

                node_name = _normalized_node_name(
                    vault.get("WORKSTATION_NAME") or vault.get("NODE_NAME") or get_workstation_identity(),
                    socket.gethostname() or "OBYLON-ENDPOINT",
                )

                wid = None
                if vault_node_id:
                    by_id = (client.table("workstations")
                             .select("id,name,hardware_uuid")
                             .eq("id", vault_node_id)
                             .limit(1)
                             .execute())
                    if by_id.data:
                        wid = str(by_id.data[0].get("id") or "") or None

                rows = []
                if not wid:
                    by_hw = (client.table("workstations")
                             .select("id,name,hardware_uuid")
                             .eq("hardware_uuid", HARDWARE_UUID)
                             .execute())
                    rows = by_hw.data or []
                    if rows:
                        candidates = sorted(str(row["id"]) for row in rows if row.get("id"))
                        wid = candidates[0] if candidates else None
                        if len(candidates) > 1:
                            logger.warning(
                                "Multiple workstation rows share one hardware UUID; reusing the canonical row.",
                                component="identity", hardware_uuid=HARDWARE_UUID,
                                duplicate_count=len(candidates), canonical_id=wid,
                            )

                fingerprint_rows, fingerprint_column_supported = ([], False)
                if not wid and live_fingerprint:
                    fingerprint_rows, fingerprint_column_supported = _server_rows_by_hardware_fingerprint(client, live_fingerprint)
                    if fingerprint_rows:
                        candidates = sorted(str(row["id"]) for row in fingerprint_rows if row.get("id"))
                        wid = candidates[0] if candidates else None
                        if len(candidates) > 1:
                            logger.warning(
                                "Multiple workstation rows share one hardware fingerprint; reusing the canonical row.",
                                component="identity", fingerprint_prefix=live_fingerprint[:16],
                                duplicate_count=len(candidates), canonical_id=wid,
                            )
                ...
                payload = {
                    ...
                    "os_info": _identity_os_info(live_fingerprint),
                }
                if fingerprint_column_supported and live_fingerprint:
                    payload["hardware_fingerprint"] = live_fingerprint
                ...
                if wid:
                    try:
                        ...
                        if "23505" in err_str or "unique constraint" in err_str.lower():
                            ...
                            raise update_error
                    if response is None:
                        raise RuntimeError("workstation update returned no response")
                else:
                    deterministic_id = _stable_node_id(HARDWARE_UUID, LICENSE_ID or vault.get("LICENSE_ID"))
                    payload["id"] = deterministic_id
                    try:
                        response = client.table("workstations").insert(payload).execute()
                        result_rows = response.data or []
                        wid = result_rows[0].get("id") if result_rows else deterministic_id
                    except Exception as insert_error:
                        # Never mint a second identity because another actor won the race.
                        by_id = (client.table("workstations")
                                 .select("id,name,hardware_uuid")
                                 ...
                                 .execute())
                        if by_id.data:
                            wid = by_id.data[0].get("id")
                            client.table("workstations").update(payload).eq("id", wid).execute()
                        else:
                            # A schema without hardware_fingerprint can still accept
                            # the os_info binding. Retry only for that schema mismatch.
                ...
                if not wid:
                    raise RuntimeError("workstation registration returned no node id")

                NODE_ID = str(wid)
                try:
                    vault._data["NODE_ID"] = NODE_ID
                    vault._data["NODE_NAME"] = node_name
                    vault._data["WORKSTATION_NAME"] = node_name
                    if live_fingerprint:
                        vault._data["HARDWARE_FINGERPRINT_AT_ACTIVATION"] = vault.get("HARDWARE_FINGERPRINT_AT_ACTIVATION") or live_fingerprint
                    vault._save()
                except Exception as save_error:
                    logger.warning("Could not persist reconciled node identity locally", component="identity", error=str(save_error))

                node_identity.save_machine_name(node_name, os.environ.get("PROGRAMDATA", "C:\\\\ProgramData"))
                node_identity.save_binding(
                    node_identity.NodeBinding(
                        node_id=NODE_ID,
                        hardware_uuid=HARDWARE_UUID,
                        hardware_fingerprint=live_fingerprint,
                        display_name=node_name,
                    ),
                    os.environ.get("PROGRAMDATA", "C:\\\\ProgramData"),
                )
                logger.info("Handshake secured", component="identity", wid=NODE_ID, node_name=node_name, hardware_bound=bool(live_fingerprint))
                return NODE_ID

            except Exception as e:
                msg = str(e).lower()
                if any(x in msg for x in ("connect", "network", "timeout", "temporarily unavailable")):
                    logger.warning(f"Network unreachable ({msg}). Retry {attempt}/{max_retries}...", component="identity")
                else:
                    logger.error(f"Supabase registration error (attempt {attempt}/{max_retries})", component="identity", error=str(e), exc_info=True)
...
def write_fastlane_rules() -> None:
    ...
    banned_window_title_keywords additionally carries FASTLANE_CRITICAL_KEYWORDS
    (2026-09) — see that constant's own comment for the exact inclusion/
    exclusion policy (adult-content brand names and unambiguous explicit
    slang only; contextual violence/self-harm/terrorism/drug/weapon terms
    stay Python-only, where the Angel Engine can actually weigh context)."""
    try:
        rules = {
        ...
        logger.info("Fast-lane rules written", component="boot",
                    ...
                    banned_title_keywords=len(rules["banned_window_title_keywords"]))
    except Exception as e:
...
# Implemented natively by Rust Core. Keeping the hotkey in the same process
# as the low-level input hook means the teacher override remains available
# even when Brain is crashed or restarting. Python has no keyboard listener.
        ...
        try:
            ...
            for chat_id in _TELEGRAM_CHAT_IDS:
                try:
                    ...
                    _robust_urlopen(req, timeout=10, component_name="telegram")
                    ...
                    if screenshot_bytes:
                        try:
                            ...
                            _robust_urlopen(photo_req, timeout=15, component_name="telegram")
                        except Exception as e:
...
def _surge_one(row) -> bool:
    ...
    try:
        ...
        # window, OR if the authenticated client was never successfully built
        # in the first place. That second case used to be invisible
...
def sync_daemon() -> None:
    ...
    while True:
        try:
            ...
            # up, the authenticated client stayed unavailable — the only place
            # that ever tried to rebuild it was _surge_one(), which only runs when
            ...
            if session_manager.get_client() is None and network_reachable():
                observation = recovery_engine.classify(
                    ...
                    reason="Supabase client unavailable while network is reachable",
                    facts={"network_online": True, "previously_licensed": bool(vault.get("LICENSE_ID"))},
                )
                recovery_engine.attempt(observation)
...
class RemoteConfigManager:
    ...
    def __init__(self, agent_id: str):
        ...
        # intentionally reads the current SessionManager client on every poll,
        # so a session recovery/rebuild is picked up immediately rather than
        # leaving this manager holding a stale authenticated client.
    ...
    def perform_update(self, download_url: str, sha256: str = None):
        ...
        try:
            ...
            if parsed.scheme.lower() != "https" or not parsed.netloc:
                logger.critical("OTA update refused: download URL must use HTTPS", component="ota")
                return False
        except Exception as e:
            logger.critical("OTA update refused: invalid download URL", component="ota", error=str(e))
            return False
        ...
        try:
            ...
            if backup_exe.exists() and current_exe.exists() and current_exe.stat().st_size < 1024*1024:
                try:
                    ...
                    logger.error("OTA rollback failed", component="ota", error=str(rollback_error), exc_info=True)
            return False
...
SEVERITY_RANK = {"info": 0, "warning": 1, "high": 1.5, "critical": 2}
R_MAX = 2

def severity_base_magnitude(category: str, k: float = 1.5, p: float = 1.5) -> float:
    # BUGFIX (2026-09): "high" used to share rank 2 with "critical" here
    # ("high": 2, "critical": 2), which pins both to the exact same base
    # magnitude (1.0) — meaning a "high"-tier lexicon hit (VPN/proxy/piracy
    # sites — LEVEL 2, "trigger log, not 30s lock" per LEXICON's own
    # comment) got the identical structural threat weight in this formula
    # as a "critical"-tier hit (explicit content), and could cross
    # CRITICAL_SEVERITY_FLOOR just as easily given enough corroboration —
    # a second, independent way mid-tier content could reach critical-level
    # enforcement, alongside the SessionAccumulator frequency issue fixed
    # above. "high" now sits at a fractional rank strictly between
    # "warning" (1) and "critical" (2) so it needs genuine amplification
    # (intent_n) to approach the ceiling, while info/warning/critical's
    # own numeric behavior is completely unchanged from before this fix —
    # only "high" moved. The exact midpoint (1.5) is a reasonable,
    # monotonic default, not a value tuned against real traffic; revisit
    # with real detection logs before treating it as final.
...
class MasterFSM:
    ...
    def process_telemetry(self, ext_payload: dict, active_proc: str, c_lev: float = 0.0,
        ...
        with _perf_section("scanner", "fsm"):
            ...
            t_score_val = max(0.0, min(1.0, _sanitize_unit_score(ext_payload.get("tripwire_score", 0.0))))
            m_score, _ = _reputation_adjusted_monetization_score(
                ext_payload.get("url_hostname", "") or ext_payload.get("url", ""), ext_payload.get("monetization_score", 0.0)
            )
            ale_score = _sanitize_ale(max(0.0, min(1.0, _sanitize_unit_score(ext_payload.get("ale_score", 0.0)))))
...
def scan_loop(workstation_id: str) -> None:
    ...
    scan_pressure = ScanPressureController()
    next_scan_interval = SCAN_INTERVAL
    ...
    while True:
        pressure_decision = scan_pressure.update(telemetry_age_sec=float("inf"), now=time.monotonic())
        next_scan_interval = SCAN_INTERVAL
        try: # THE GLOBAL SHIELD
            ...
            if (title_str, proc_str) != last_reported_window:
                try:
                    if session_manager.get_client() is not None:
                        ...
                        last_reported_window = (title_str, proc_str)
                except Exception:
                    pass
                    ...
                    if not exemption_found:
                        try:
                            ...
                            if pyperclip is None:
                                import pyperclip as _pyperclip
                                pyperclip = _pyperclip
                            pyperclip.copy("[OBYLON: BULK DATA TRANSFER BLOCKED BY DPDP COMPLIANCE POLICY]")
                        except Exception: pass
                        DPDP.is_hot = False
            ...
            # ==========================================
            # ADAPTIVE OBSERVATION PRESSURE
            # ==========================================
            ale_score = _sanitize_ale(v7_payload.get("ale_score", 0.0)) if 'v7_payload' in locals() and v7_payload else 0.0
            m_score_raw = v7_payload.get("monetization_score", 0.0) if 'v7_payload' in locals() and v7_payload else 0.0
            m_score, reputation_meta = _reputation_adjusted_monetization_score(
                (v7_payload.get("url_hostname", "") or browser_url) if 'v7_payload' in locals() and v7_payload else browser_url,
                m_score_raw,
            )
            t_score_val = max(0.0, min(1.0, _sanitize_unit_score(v7_payload.get("tripwire_score", 0.0)))) if 'v7_payload' in locals() and v7_payload else 0.0
            reputation_pressure = reputation_meta.get("contextual_score", 0.0) if reputation_meta else 0.0
            pressure_decision = scan_pressure.update(
                reputation=reputation_pressure,
                monetization=m_score,
                lexical=c_lev,
                dom=c_dom,
                ale=ale_score,
                tripwire=t_score_val,
                ocr=c_ocr,
                telemetry_age_sec=telemetry_age(),
            )
            next_scan_interval = max(SCAN_PRESSURE_MIN_INTERVAL, pressure_decision.interval_sec)
            if pressure_decision.state is not ScanPressure.NORMAL:
                logger.info(
                    "Adaptive scan pressure",
                    component="scan",
                    state=pressure_decision.state.value,
                    pressure=f"{pressure_decision.score:.2f}",
                    interval=f"{pressure_decision.interval_sec:.2f}",
                    reputation=f"{reputation_pressure:.2f}",
                    telemetry_age=f"{telemetry_age():.1f}",
            ...
            if s_final == 0.0 and (c_lev >= ENFORCEMENT_FLOOR or c_dom > 0.0):
                    ...
                    # Pressure-aware OCR admission. Existing investigation paths
                    # keep their evidence capture; elevated/aggressive pressure
                    # controls whether additional OCR/evidence sampling occurs.
                    if volatile_ram_snapshot is not None and (pressure_decision.ocr_due or c_lev > 0.70 or t_score_val > 0.40):
                        if OCR.submit(volatile_ram_snapshot):
                            scan_pressure.mark_ocr()

                    if pressure_decision.state is ScanPressure.AGGRESSIVE and pressure_decision.ocr_due and volatile_ram_snapshot is None and browser_family(proc_str) is not None:
                        pressure_capture = capture_screenshot()
                        if pressure_capture and OCR.submit(pressure_capture):
                            scan_pressure.mark_ocr()
                        if pressure_capture and pressure_decision.evidence_due and s_final < ENFORCEMENT_FLOOR:
                            volatile_ram_snapshot = pressure_capture
                            scan_pressure.mark_evidence()
            ...
            if c_lev > 0.0 or c_dom > 0.0 or c_ocr > 0.0 or typed_hit:
                logger.info("Matrix stats", component="telemetry", lev=f"{c_lev:.2f}", dom=f"{c_dom:.2f}", ocr=f"{c_ocr:.2f}", appmod=f"{m_app:.2f}", final=f"{s_final:.2f}", hit=best_hit, scan_pressure=(pressure_decision.state.value if 'pressure_decision' in locals() else "normal"))
        ...
        time.sleep(max(SCAN_PRESSURE_MIN_INTERVAL, next_scan_interval))

# Boot control-plane state. Registration is deliberately asynchronous: a slow
# Supabase lookup must never hold Brain's realtime/control startup hostage.
_BOOT_WID_LOCK = threading.RLock()
_BOOT_REGISTERED_WID: str | None = None
_BOOT_REGISTRATION_STARTED = False


def _record_boot_wid(wid: str | None) -> None:
    global _BOOT_REGISTERED_WID, NODE_ID
    if not wid or str(wid).startswith("offline-"):
        return
    with _BOOT_WID_LOCK:
        _BOOT_REGISTERED_WID = str(wid)
        NODE_ID = str(wid)


def resolve_offline_wid(current_wid: str) -> str:
    """Return the real workstation id once asynchronous registration has it.

    This function sits in the 1-second scanner and realtime reconnect paths.
    It must never perform a network registration itself.
    """
    with _BOOT_WID_LOCK:
        registered = _BOOT_REGISTERED_WID
    return registered or current_wid


def _background_registration_loop() -> None:
    """Reconcile workstation identity without blocking Brain startup."""
    global _BOOT_REGISTRATION_STARTED
    with _BOOT_WID_LOCK:
        if _BOOT_REGISTRATION_STARTED:
            return
        _BOOT_REGISTRATION_STARTED = True
    ...
    while True:
        try:
            ...
            if client is None:
                logger.info(
                    "Workstation registration deferred until authenticated client is available",
                    component="identity",
                )
            else:
                new_wid = register_workstation()
                if new_wid and not str(new_wid).startswith("offline-"):
                    _record_boot_wid(new_wid)
                    try:
                        write_identity_beacon(new_wid)
                    except Exception as e:
                        logger.warning(
                            "Identity beacon refresh failed after async registration",
                            component="identity",
                            error=str(e),
                        )
                    logger.info(
                        "Asynchronous workstation registration completed",
                        component="identity",
                        wid=new_wid,
                    )
                    return
            ...
            logger.warning(
                "Asynchronous workstation registration attempt failed",
                component="identity",
                error=str(e),
            )
...
def controlled_shutdown(workstation_id: str, action_id: str):
    """Initiate an immediate endpoint shutdown after best-effort evidence
    capture has been launched in parallel. The command itself stays on the
    Realtime -> Brain -> Core control path and never sleeps for evidence.
    """
    ...
    def _upload_cam():
        try:
                ...
                if url:
                    try:
                        ...
                        if client is not None:
                            client.table("evidence_logs").insert({
                                "metadata": {
                                    "command": "terminate",
                                    "action_id": action_id,
                                    "is_backlogged": False,
            ...
            logger.warning("Shutdown webcam evidence failed", component="admin", error=str(e))
    ...
    def _upload_screen():
        try:
            ...
            logger.warning("Shutdown screenshot evidence failed", component="admin", error=str(e))

    threading.Thread(target=_upload_cam, daemon=True, name="shutdown_evidence_cam").start()
    threading.Thread(target=_upload_screen, daemon=True, name="shutdown_evidence_screen").start()

    if platform.system() == "Windows":
        try:
            resp = _core_ipc_call({"cmd": "shutdown"}, timeout=1.5)
            if resp.get("ok"):
                logger.info("Immediate shutdown accepted by ObylonCore", component="admin", action_id=action_id)
                return True
            logger.error("ObylonCore rejected immediate shutdown", component="admin", response=resp)
        except Exception as e:
            logger.warning("Core immediate shutdown path unavailable; using direct fallback", component="admin", error=str(e))

        # Fail-safe compatibility fallback. Popen returns as soon as Windows
        # accepts the request instead of synchronously waiting for shell work.
        try:
            subprocess.Popen(
            ...
            logger.info("Immediate shutdown fallback launched", component="admin", action_id=action_id)
            return True
        except Exception as e:
            logger.error("Immediate Windows shutdown failed", component="admin", error=str(e), action_id=action_id)
            return False
    ...
    try:
        ...
        logger.error("System shutdown failed", component="admin", error=str(e), action_id=action_id)
        return False
...
def execute_command(cmd: str) -> bool:
    ...
    if cmd == "lock":
        try:
            if system == "Windows":
                return subprocess.call("rundll32.exe user32.dll,LockWorkStation", shell=True) == 0
            if system == "Darwin":
                return subprocess.call(["pmset", "displaysleepnow"]) == 0
            return subprocess.call(["loginctl", "lock-session"]) == 0
        except Exception as e:
            logger.error("Lock command failed", component="admin", error=str(e))
            return False
...
def show_classroom_focus_overlay() -> bool:
    try:
...
def hide_classroom_focus_overlay() -> bool:
    try:
    ...
    async def _run_realtime():
        ...
        while True:
            ...
            try:
                ...
                fresh_token, _ = session_manager.get_tokens()
                if not fresh_token:
                    raise RuntimeError("authenticated session disappeared during Realtime connect")
                await client.set_auth(fresh_token)
                ...
                # BUGFIX (2026-09): the two listeners above only ever fire on
                # a change to the shared `licenses` row — they have nothing
                # to say about THIS node specifically being removed. A
                # school admin removing a single workstation (e.g. to
                # relocate it) never touches the `licenses` row at all, so
                # this agent had no realtime signal for "you were just
                # deprovisioned" and would keep running until the next
                # /license_heartbeat poll happened to notice (and, until
                # the fix in license_heartbeat_loop below, it wouldn't even
                # have noticed then — see that function's comment).
                # `db_license_node_reclaim_fix.sql` adds an AFTER DELETE
                # trigger on `workstations` that flips this node's own
                # license_nodes.status to 'reclaimed' the moment it's
                # removed from the dashboard; this listens for that
                # specifically, scoped to this node's own row (RLS now
                # limits this to SELECT on rows where auth_user_id is this
                # node's own auth user, so it can't see other nodes' rows).
                def _on_node_reclaimed(payload):
                    record = payload.get("record") or payload.get("new") or {}
                    status = record.get("status")
                    if status and status != "active":
                        logger.critical(f"Realtime C2: this node's license seat was {status} (likely removed/relocated from the dashboard). Initiating shutdown.", component="realtime")
                        LICENSE_INVALID_EVENT.set()
                ...
                license_channel.on_postgres_changes(
                    ...
                    schema="public",
                    table="license_nodes",
                    filter=f"id=eq.{workstation_id}",
                    callback=_on_node_reclaimed
                )

                # BUGFIX (2026-09): license_nodes.license_id is ON DELETE
                # CASCADE to licenses.id (see obylon_master_template.sql)
                # — deleting the parent license outright doesn't UPDATE
                # this node's row, it deletes it along with the license.
                # The UPDATE-only listener above has nothing to catch
                # there. This node's row disappearing entirely, for any
                # reason, is unconditionally fatal — there's no status
                # left to read, so unlike _on_node_reclaimed this doesn't
                # need to inspect the payload at all: the filter already
                # guarantees this event is about this node's own row.
                def _on_node_deleted(payload):
                    logger.critical("Realtime C2: this node's license_nodes row was deleted (its license was likely deleted outright). Initiating shutdown.", component="realtime")
                    LICENSE_INVALID_EVENT.set()
                ...
                license_channel.on_postgres_changes(
                    ...
                    table="license_nodes",
                    filter=f"id=eq.{workstation_id}",
                    callback=_on_node_deleted
                )
                
                await license_channel.subscribe()
                REALTIME_CONNECTED.set()
                logger.info("Realtime channels subscribed and healthy", component="realtime")
                ...
                def _on_insert(payload):
                    ...
                    def _handle():
                        ...
                        try:
                            ...
                            if not str(action_id).startswith("broadcast-"):
                                if session_manager.get_client() is None:
                                    ...
                                    # until something else happened to rebuild the client.
                                ...
                                # Local arbitration is authoritative for the two
                                # consumers in this process. Do not infer ownership
                                # from UPDATE+SELECT response rows because RLS can
                                # make a successful update look like an empty claim.
                                if not _claim_admin_action(str(action_id)):
                                    return
                            ...
                            # --- COMMAND DISPATCH ---
                            if cmd == "terminate":
                                dispatched = bool(controlled_shutdown(workstation_id, action_id))
                                ...
                                if not WARDEN:
                                    ...
                                    try:
                                        ...
                                        duration = 300
                                    dispatched = WARDEN.lock_workstation(duration=duration, force=True)

                            elif cmd == "unfreeze":
                                if not WARDEN:
                                    dispatched, fail_reason = False, "WARDEN not initialized on this workstation"
                                else:
                                    dispatched = WARDEN.disengage_freeze()

                            elif cmd == "classroom_focus":
                                # Global classroom attention mode
                                if WARDEN:
                                    dispatched = WARDEN.lock_workstation(duration=3600, force=True, reason="classroom_focus")
                                else:
                                    dispatched = False
                                dispatched = bool(dispatched and show_classroom_focus_overlay())
                                if not dispatched:
                                    fail_reason = "Warden rejected classroom_focus (likely due to pending security init or higher priority freeze)"
                                ...
                                if WARDEN:
                                    ...
                                    dispatched = WARDEN.disengage_freeze(reason="classroom_focus")
                                else:
                                    dispatched = True
                                dispatched = bool(dispatched and hide_classroom_focus_overlay())
                                if not dispatched:
                                    fail_reason = "Warden rejected classroom_focus_end"
                                ...
                                if not WARDEN:
                                    ...
                                    if target_proc:
                                        dispatched = WARDEN.terminate_process(target_proc)
                                    else:
                                ...
                                new_alias = _normalized_node_name(meta.get("alias") or meta.get("new_name") or meta.get("name"), "")
                                if new_alias and new_alias != "OBYLON-ENDPOINT":
                                    try:
                                        ...
                                        if platform.system() == "Windows":
                                            subprocess.call(["attrib", "+H", str(ALIAS_FILE)], shell=False)
                                        global WORKSTATION_NAME, NODE_ID
                                        WORKSTATION_NAME = new_alias
                                        NODE_ID = workstation_id
                                        vault._data["NODE_NAME"] = new_alias
                                        vault._data["WORKSTATION_NAME"] = new_alias
                                        vault._data["NODE_ID"] = workstation_id
                                        vault._save()
                                        node_identity.save_machine_name(new_alias, os.environ.get("PROGRAMDATA", "C:\\ProgramData"))
                                        try:
                                            binding = node_identity.load_binding(os.environ.get("PROGRAMDATA", "C:\\ProgramData"))
                                            if binding:
                                                node_identity.save_binding(node_identity.NodeBinding(
                                                    node_id=binding.node_id, hardware_uuid=binding.hardware_uuid,
                                                    hardware_fingerprint=binding.hardware_fingerprint, display_name=new_alias
                                                ), os.environ.get("PROGRAMDATA", "C:\\ProgramData"))
                                        except Exception:
                                            pass
                            ...
                            # Finalize — ephemeral broadcasts don't touch the DB
                            if not str(action_id).startswith("broadcast-"):
                ...
                # Fast Realtime Presence (bypasses 15s DB poll for the Web UI)
                grid_channel = client.channel("sentinel-grid")
                await grid_channel.subscribe()
                await grid_channel.track({
                    "workstation_id": workstation_id,
                    "status": "online"
                })

                logger.info("Realtime C2 channel subscribed (CDC + Broadcast + Global + Presence)", component="realtime", target=workstation_id)
                ...
                while True:
                    if TOKEN_ROTATED_EVENT.is_set():
                        ...
                        # Disconnect safely to trigger the outer reconnect loop, which will read
                            # the newest token directly from SessionManager.
                        try:
                            await client.close()
                        except Exception:
                            pass
            ...
            for action in res.data or []:
                ...
                try:
                    ...
                    if created and (now - created) > timedelta(seconds=COMMAND_TTL_SEC):
                        ...
                        continue                    # Local arbitration prevents Realtime and polling from
                    # dispatching the same row twice without depending on a
                    # SELECT-return policy after an UPDATE.
                    if not _claim_admin_action(str(action["id"])):
                        continue
                    ...
                    if cmd == "terminate":
                        dispatched = bool(controlled_shutdown(workstation_id, action["id"]))
                        ...
                        if not WARDEN:
                            ...
                            if target:
                                dispatched = WARDEN.terminate_process(target)
                            else:
                                dispatched, fail_reason = False, "No target provided in metadata"
...
# ---------- Asynchronous boot services ----------
_IDENTITY_BOOT_STARTED = False
_SESSION_BOOT_STARTED = False
_BOOT_IDENTITY_STOP = threading.Event()
_BOOT_SESSION_STOP = threading.Event()


def _signal_core_security_ready() -> None:
    try:
        response = _core_ipc_call({"cmd": "brain_security_ready"}, timeout=1.0)
        if response.get("ok"):
            logger.info("Signaled security-ready to Core.", component="identity")
        else:
            logger.warning("Core refused security-ready signal", component="identity", response=response)
    except Exception as e:
        logger.warning("Core security-ready signal deferred", component="identity", error=str(e))


def _identity_verification_loop() -> None:
    """Verify hardware identity off the startup critical path.

    A transient helper failure is an unavailable observation and is retried in
    the background. A real mismatch remains terminal and fails closed via the
    same exit code Core already treats as a non-restartable security decision.
    """
    global _IDENTITY_BOOT_STARTED
    with _BOOT_WID_LOCK:
        if _IDENTITY_BOOT_STARTED:
            return
        _IDENTITY_BOOT_STARTED = True

    # The first attempt is deliberately short. The helper itself is allowed to
    # time out, but Brain never waits on it. Later attempts use a slow backoff.
    delay = 2.0
    attempt = 0
    while not _BOOT_IDENTITY_STOP.is_set():
        attempt += 1
        try:
            # WMI queries can easily take 10-15s on some systems (especially
            # after boot). Because this loop runs in a background thread, we
            # can safely give the helper FINGERPRINT_HELPER_TIMEOUT_SEC
            # (15s) — comfortably above the Go helper's own 12s internal
            # WMI/CIM deadline — rather than aggressively killing the probe
            # (a shorter Python-side timeout than the Go helper's own
            # deadline meant subprocess.run() was routinely killing the
            # helper mid-query on cold boots, which is exactly what
            # produced repeated "hardware fingerprint helper timed out" /
            # "failed" attempts before an eventual, later success).
            fp = get_hardware_fingerprint(timeout=FINGERPRINT_HELPER_TIMEOUT_SEC)
            if not _is_hardware_fingerprint(fp):
                stored_fp = vault.get("HARDWARE_FINGERPRINT_AT_ACTIVATION")
                if stored_fp:
                    sys_state.update_security(
                        SecurityState.UNKNOWN,
                        "Live hardware fingerprint unavailable",
                        "identity",
                        next_act="background retry",
                    )
                logger.warning(
                    "Hardware identity verification pending; background probe unavailable",
                    component="identity",
                    attempt=attempt,
                )
            else:
                # A real, format-valid live reading exists now — make it
                # available to get_hardware_fingerprint_blocking() callers
                # (register_workstation(), recover_identity) immediately,
                # independent of whether it goes on to validate against the
                # stored activation fingerprint below.
                _hardware_fingerprint_result["value"] = fp
                _hardware_fingerprint_ready.set()
                if not validate_identity_integrity(vault, fp):
                    if sys_state.security == SecurityState.IDENTITY_MISMATCH:
                        logger.critical(
                            "Hardware identity mismatch detected after background verification; stopping Brain.",
                            component="identity",
                        )
                        os._exit(78)
                    # Stored identity exists but live proof disappeared between
                    # attempts. Remain pending and retry rather than terminating.
                    logger.warning(
                        "Hardware identity verification incomplete; retaining security-pending state",
                        component="identity",
                    )
                else:
                    _promote_provisional_hardware_uuid(fp)
                    if sys_state.security == SecurityState.READY:
                        _signal_core_security_ready()
                        logger.info(
                            "Hardware identity verification completed asynchronously",
                            component="identity",
                            attempt=attempt,
                        )
                        return
                    if sys_state.security == SecurityState.LEGACY_UNVERIFIED:
                        # There is no activation fingerprint to compare. Preserve
                        # legacy compatibility, but never claim verified security.
                        logger.warning(
                            "Legacy workstation has no activation fingerprint; identity verification remains unverified",
                            component="identity",
                        )
                        return
        except Exception as e:
            logger.warning("Background hardware identity probe failed", component="identity", error=str(e))

        _BOOT_IDENTITY_STOP.wait(delay)
        delay = min(delay * 1.5, 30.0)


def _session_bootstrap_loop() -> None:
    """Create/restore the Supabase session without blocking Brain startup."""
    global _SESSION_BOOT_STARTED
    if _SESSION_BOOT_STARTED:
        return
    _SESSION_BOOT_STARTED = True
    delay = 0.5
    while not _BOOT_SESSION_STOP.is_set():
        ...
        if sys_state.auth == AuthState.AUTH_TERMINAL:
            time.sleep(15)
            observation = recovery_engine.classify(
                signal="auth_terminal_recovery",
                component="auth",
                reason="Waiting for new activation credentials after terminal auth failure",
                facts={"network_online": network_reachable(), "previously_licensed": bool(vault.get("LICENSE_ID"))}
            )
            recovery_engine.attempt(observation)
            if vault.get("ACCESS_TOKEN"):
                sys_state.update_auth(AuthState.UNINITIALIZED, "Vault reloaded", "session_mgr")
            continue
        try:
            ok = bool(session_manager.initialize_from_vault())
            if ok or session_manager.get_client() is not None:
                logger.info("Supabase session bootstrap completed", component="auth")
                return
        except Exception as e:
            logger.warning("Supabase session bootstrap failed", component="auth", error=str(e))
        _BOOT_SESSION_STOP.wait(delay)
        delay = min(delay * 2.0, 8.0)


def _boot_license_check_once() -> None:
    """Best-effort online license check. Runs after local protection is armed."""
    try:
        import ssl, certifi, urllib.request, json
        access_token, _ = session_manager.get_tokens()
        if not access_token:
            logger.debug("Boot license check deferred: no authenticated token", component="license")
            return
        ...
        ctx = ssl.create_default_context(cafile=certifi.where())
        response = _robust_urlopen(req, timeout=5, component_name="license")
        with response:
            data = json.loads(response.read().decode("utf-8"))
        if "server_sig" in data and not verify_server_signature(data, data["server_sig"], hardware_uuid=HARDWARE_UUID):
        ...
        if status:
            sys_state.update_license(LicenseState[status.upper()] if status.upper() in LicenseState.__members__ else LicenseState.UNKNOWN, "Boot heartbeat response", "license")
            vault._data["LICENSE_STATUS"] = status
        if data.get("issued_at"):
            vault._data["LAST_HEARTBEAT_OK_AT"] = data["issued_at"]
            current_max = vault._data.get("MAX_SEEN_UTC")
            if not current_max or data["issued_at"] > current_max:
                vault._data["MAX_SEEN_UTC"] = data["issued_at"]
        if data.get("expires_at"):
            vault._data["EXPIRES_AT"] = data["expires_at"]
        if data.get("grace_days"):
            vault._data["GRACE_DAYS"] = data["grace_days"]
        vault._save()
        if status in ("revoked", "suspended", "expired", "deactivated", "reclaimed"):
            logger.critical(f"License is currently {status}; invalidation signaled.", component="license")
            LICENSE_INVALID_EVENT.set()
    except Exception as e:
        logger.debug("Boot-time license check unavailable; continuing with cached license state", component="license", error=str(e))


def _boot_license_watch() -> None:
    time.sleep(0.1)
    _boot_license_check_once()
...
def license_heartbeat_loop(workstation_id: str):
    while True:
        try:
            ...
            access_token, _ = session_manager.get_tokens()
            if not access_token:
                raise RuntimeError("No authenticated access token available for license heartbeat")
            ...
            # Robust proxy handling: Try with system proxies first
            response = _robust_urlopen(req, timeout=30.0, component_name="license")
            ...
            with response:
                ...
                # BUGFIX (2026-09): "deactivated"/"reclaimed" are
                # license_nodes' own status vocabulary (see
                # license_nodes_status_check in the schema) — a node
                # removed from the dashboard lands here, not in
                # "revoked"/"suspended"/"expired" (those describe the
                # shared license record, not a single node). Whatever
                # /license_heartbeat actually forwards, both vocabularies
                # need to be treated as "stop".
                if status in ("revoked", "suspended", "expired", "deactivated", "reclaimed"):
                ...
                # BUGFIX (2026-09): the check above trusts whatever
                # /license_heartbeat chose to return. This second check
                # asks the database directly, with this node's own
                # authenticated session, for the actual license_nodes row
                # this agent IS — ground truth, independent of anything
                # that edge function does or doesn't forward. RLS now
                # limits this to SELECT on this node's own row only (see
                # db_license_node_reclaim_fix.sql), so it can't be used to
                # snoop on other nodes.
                if NODE_ID:
                    try:
                        node_row = (
                        ...
                        # Best-effort corroboration only — the primary
                        # /license_heartbeat check above already ran; don't
                        # let a transient read failure here (network,
                        # auth-refresh race, etc.) override a good
                        # heartbeat result.
                        logger.debug(f"Direct license_nodes check unavailable: {e}", component="license")
                    else:
                        # BUGFIX (2026-09): .maybe_single() returns
                        # data=None WITHOUT raising when zero rows match —
                        # that's the same "confirmed gone" signal as the
                        # 404 handled above (license_id is ON DELETE
                        # CASCADE to licenses.id, so deleting the license
                        # hard-deletes this row too, leaving no status to
                        # read). The original version of this check only
                        # looked at node_status != "active", which is
                        # None for BOTH "row doesn't exist" and "query
                        # didn't run" — it silently did nothing for a
                        # confirmed-deleted row, the exact same failure
                        # shape as the 404 bug above, just reached via the
                        # direct-read path instead of the HTTP path.
                        node_status = (node_row.data or {}).get("status") if node_row.data else None
                        if node_row.data is None:
                            logger.critical("This node's license_nodes row no longer exists (license was likely deleted outright). Shutting down.", component="license")
                            sys_state.update_license(LicenseState.REVOKED, "license_nodes row not found", "heartbeat")
                            LICENSE_INVALID_EVENT.set()
                            return
                        if node_status and node_status != "active":
                            logger.critical(f"This node's license_nodes row is '{node_status}' (removed/relocated from the dashboard). Shutting down.", component="license")
                            sys_state.update_license(LicenseState.REVOKED, f"license_nodes.status={node_status}", "heartbeat")
                            LICENSE_INVALID_EVENT.set()
                            return
            ...
            # server looked this node up (by hardware_uuid) or its parent
            # license up (by license_id) and found NOTHING — that's
            # exactly what this endpoint returns once either row is gone.
            # license_nodes.license_id is ON DELETE CASCADE to licenses.id
            # (see obylon_master_template.sql), so deleting a license
            # outright hard-deletes every one of its nodes' rows with it —
            # there's no status to read, the row simply isn't there. A 404
            # is a permanent, authoritative "you're gone" signal, not a
            # network hiccup, and must never get the grace-period
            # tolerance below. Before this fix it fell straight into the
            # generic branch here and into "Offline tolerance" (comparing
            # against the last successful heartbeat), so deleting a
            # license outright left the agent running fully functional
            # for up to GRACE_DAYS (14 by default) — the handler simply
            # couldn't tell "the internet is down" apart from "the server
            # explicitly says you don't exist."
            if isinstance(e, urllib.error.HTTPError) and e.code == 404:
                try:
                    body = json.loads(e.read().decode("utf-8"))
                except Exception:
                    body = {}
                logger.critical(f"License heartbeat got 404 ({body.get('error', 'not found')}) — this node or its license no longer exists server-side. Shutting down.", component="license")
                sys_state.update_license(LicenseState.REVOKED, "License/node not found (404)", "heartbeat")
                LICENSE_INVALID_EVENT.set()
                return
            ...
            # Smart recovery: If we don't have a token, maybe the CLI activated us?
            access_token, _ = session_manager.get_tokens()
            if not access_token:
                vault.load()
...
def harden_installation():
    ...
    # This runs as SYSTEM during boot via the broker (rust/broker/src/main.rs
    # ensure_acls(), which is the authoritative copy of this list and also
    # widens the logs/capture/events *directories* with inheritance — this
    # Python copy is a redundant safety net and, running as the non-admin
    # session user itself, usually can't actually change another file's ACL
    # (needs WRITE_DAC, which this process doesn't have on a SYSTEM-owned
    # file) — kept in sync mainly so the two lists don't drift.
    #
    # SECURITY/RELIABILITY FIX (2026-09): this list used to be just
    # obylon.enc/identity_beacon.json/fastlane_rules.json, with a comment
    # claiming ".machine_id and license_seed.txt stay student-read-only."
    # That was only half true: license_seed.txt genuinely is read-only (see
    # the zero-touch fleet ignition path, which only ever reads it), but
    # .machine_id is written directly by this same process a few hundred
    # lines below (IDENTITY_FILE.write_text(...)) — the comment and the
    # code disagreed about who writes it. node_binding.json/node_name
    # (node_identity.py) and recovery_journal.json have the same problem:
    # written by the interactive-user Brain process, never previously
    # ACL-widened. license_seed.txt stays off this list on purpose.
    ...
    files_to_grant = [
        ...
        vault_dir / "node_binding.json",
        vault_dir / "node_name",
        vault_dir / ".machine_id",
        vault_dir / "recovery_journal.json",
    ]
...
def _configure_recovery_engine(resurrect_fn=None, core_systems_ref=None, active_threads_ref=None) -> None:
    """Bind Recovery Intelligence to explicit, pre-approved Brain operations.

    The integration layer supplies callables. The engine never receives shell
    commands, module names, or arbitrary code from telemetry.
    """
    recovery_engine.register_action("recover_auth", lambda obs: session_manager.force_refresh())
    recovery_engine.register_action(
        "recover_client",
        lambda obs: session_manager.force_refresh() or session_manager.initialize_from_vault(),
    )

    def _recover_identity(_obs):
        wid = register_workstation()
        return bool(wid) and not str(wid).startswith("offline-")

    recovery_engine.register_action("recover_identity", _recover_identity)
    recovery_engine.register_action("recover_identity_relink", _recovery_relink_existing_activation)
    recovery_engine.register_action("recover_vault", lambda obs: vault.load())
    recovery_engine.register_action(
        "recover_sync",
        lambda obs: session_manager.force_refresh() if session_manager.get_client() is None else True,
    )
    def _recover_realtime(_obs):
        TOKEN_ROTATED_EVENT.set()
        return REALTIME_CONNECTED.wait(timeout=5.0)

    recovery_engine.register_action("recover_realtime", _recover_realtime)

    def _recover_thread(obs):
        if resurrect_fn is None or core_systems_ref is None:
            return False
        config = core_systems_ref.get(obs.component)
        if not config:
            return False
        resurrect_fn(obs.component, config)
        return True

    recovery_engine.register_action("recover_thread", _recover_thread)

    recovery_engine.register_validator("verify_auth", lambda obs: sys_state.auth == AuthState.AUTHENTICATED)
    recovery_engine.register_validator("verify_client", lambda obs: session_manager.get_client() is not None)
    recovery_engine.register_validator(
        "verify_license",
        lambda obs: sys_state.license not in {LicenseState.REVOKED, LicenseState.SUSPENDED, LicenseState.EXPIRED},
    )
    recovery_engine.register_validator("verify_identity", lambda obs: sys_state.security == SecurityState.READY)
    recovery_engine.register_validator(
        "verify_identity_relinked",
        lambda obs: sys_state.security == SecurityState.READY,
    )
    recovery_engine.register_validator("verify_vault", lambda obs: vault.load())
    recovery_engine.register_validator("verify_sync", lambda obs: sys_state.is_ready_for_sync())
    recovery_engine.register_validator("verify_realtime", lambda obs: REALTIME_CONNECTED.is_set())

    def _verify_thread(obs):
        if active_threads_ref is None:
            return False
        thread = active_threads_ref.get(obs.component)
        return bool(thread and thread.is_alive())

    recovery_engine.register_validator("verify_thread", _verify_thread)



def _recovery_relink_existing_activation() -> bool:
    """Relink an already-licensed machine; never fabricate a license."""
    wid = register_workstation()
    if not wid or str(wid).startswith("offline-"):
        return False
    try:
        ...
        logger.warning("Identity beacon refresh failed during recovery", component="recovery", error=str(e))
    return True


def _reload_recovery_configuration() -> bool:
    """Reload only the normal configuration path; no arbitrary file execution."""
    try:
        load_remote_config()
        return True
    except Exception:
...
def main() -> None:
    global HARDWARE_UUID, WORKSTATION_NAME, LICENSE_ID, NODE_ID, SUPABASE_URL, SUPABASE_KEY
    try:
        ...
        # Core is reachable and local vault initialization has completed. This
        # is a local worker gate, not the security-ready signal. Core itself
        # continues to reject protected IPC until the identity verifier signals
        # brain_security_ready.
        CORE_READY.set()

        # Backend registration is intentionally not joined here. A persisted
        # NODE_ID is already enough to establish the control-plane target, and a
        # fresh/offline endpoint can temporarily use an offline id while the
        # background registrar reconciles it with Supabase. The control-plane
        # worker set is allowed to start even while auth/identity are recovering.
        with _BOOT_WID_LOCK:
            registered_wid = _BOOT_REGISTERED_WID

        persisted_node_id = str(vault.get("NODE_ID") or NODE_ID or "").strip()
        wid = registered_wid or persisted_node_id or f"offline-{HARDWARE_UUID}"
        if persisted_node_id:
            _record_boot_wid(persisted_node_id)
            wid = persisted_node_id
        elif registered_wid:
            wid = registered_wid

        logger.info(
            "Workstation control identity available for immediate startup.",
            component="identity",
            wid=wid,
        )

        # Security-ready is earned, never assumed. Legacy deployments may report
        # READY as unverified, but unavailable/corrupt/mismatched identity never
        # crosses the Core readiness boundary.
        if sys_state.security == SecurityState.READY:
            try:
            ...
            logger.critical("Identity validation incomplete; Core security-ready withheld.", component="boot", security_state=sys_state.security.name)

        logger.info("Boot critical path complete; starting worker set", component="boot")
        ...
        _configure_recovery_engine(resurrect, core_systems, active_threads)
        ...
        try:
            while True:
                ...
                if LICENSE_INVALID_EVENT.is_set():
                    ...
                    os._exit(0) # Exit with 0 to prevent Windows Task Scheduler RestartOnFailure from looping
                ...
                try:
                    for name, thread in list(active_threads.items()):
                        if not thread.is_alive():
                            ...
                            # Route thread recovery through the bounded Recovery Intelligence
                            # engine instead of unconditionally resurrecting forever.
                            observation = recovery_engine.classify(
                                signal="thread_flatlined",
                                component=name,
                                reason=error_msg,
                                facts={
                                    "thread_failed": True,
                                    "network_online": sys_state.network == NetworkState.ONLINE,
                                },
                            )
                            outcome = recovery_engine.attempt(observation)
                            if outcome == RecoveryOutcome.RECOVERED:
                                logger.info("Recovery engine restarted failed subsystem", component="lazarus", thread=name)
                            elif outcome in (RecoveryOutcome.BUDGET_EXHAUSTED, RecoveryOutcome.BLOCKED, RecoveryOutcome.FAILED):
                                logger.critical("Subsystem recovery not available", component="lazarus", thread=name, outcome=outcome.value)
...
if __name__ == "__main__":
    # Background identity verification is started from the standard boot path
    # only after the vault has been loaded. There must be exactly one helper
    # probe in flight at a time; duplicate PowerShell/CIM probes can contend
    # with the same Windows management stack and make the very race we are
    # trying to recover from worse.
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--warmup":
        ...
        _warmup_step("Input hook (native Rust Core)", lambda: None)  # Keyboard hook is in Core; no Python import needed
    ...
    try:
        ...
        # A normal endpoint gets exactly one local vault load. A stale signed vault
        # gets one bounded online repair. Missing/corrupt state never enters an
        # endless 15/60-second loop, because that merely turns one recoverable
        # condition into a supervisor restart storm.
        vault_ok = bool(vault.load() and vault.get("ACCESS_TOKEN"))
        if not vault_ok and vault.last_signature_invalid:
            logger.warning("Cached signed vault is stale/invalid; attempting one authenticated repair", component="vault")
            vault_ok = bool(vault.repair_signed_state_from_server() and vault.get("ACCESS_TOKEN"))

        if not vault_ok:
            seed_file = Path(os.environ.get('PROGRAMDATA', 'C:\\ProgramData')) / "Obylon" / "license_seed.txt"
            if seed_file.exists():
                logger.info("Found license_seed.txt. Initiating bounded zero-touch fleet ignition...", component="system")
                seed_key = seed_file.read_text(encoding="utf-8").strip()
                import platform
                hostname = platform.node()
                status = "UNAVAILABLE"
                for attempt in range(1, 4):
                    hardware_fingerprint = get_verified_hardware_fingerprint(max_attempts=1)
                    if not _is_hardware_fingerprint(hardware_fingerprint):
                        logger.warning("Stable hardware identity unavailable for fleet ignition", component="identity", attempt=attempt)
                    ...
                    if status == "SUCCESS":
                        ...
                        break
                    logger.warning("Fleet ignition attempt failed", component="system", attempt=attempt, status=status)
                    time.sleep(min(5 * attempt, 10))
            if not vault_ok:
                logger.critical(
                    "Vault is unavailable or authentication state is invalid. Entering safe hold pattern.",
                    component="system",
                    last_error=vault.last_load_error,
                )
                # DO NOT sys.exit(78). If the PC is offline and its signature is stale, exiting will cause
                # a service restart storm. Instead, idle here and periodically retry repair.
                while not vault_ok:
                    time.sleep(30)
                    if vault.last_signature_invalid:
                        vault_ok = bool(vault.repair_signed_state_from_server() and vault.get("ACCESS_TOKEN"))
                    else:
                        vault_ok = bool(vault.load() and vault.get("ACCESS_TOKEN"))
                logger.info("Vault recovered successfully.", component="system")
        # A3: Clone detection / identity integrity is deliberately asynchronous.
        # The live fingerprint is still required before Core accepts protected
        # IPC, but a slow helper must never delay Brain's network/control startup.
        threading.Thread(target=_identity_verification_loop, daemon=True, name="identity_verify").start()
        ...
        # the entire Brain process before Realtime can even be attempted.
        threading.Thread(target=_session_bootstrap_loop, daemon=True, name="session_bootstrap").start()

        # Set global vars for telemetry mapping immediately from durable local state.
        HARDWARE_UUID = vault.get("HARDWARE_UUID") or HARDWARE_UUID
        WORKSTATION_NAME = get_workstation_identity(vault.get("WORKSTATION_NAME") or vault.get("NODE_NAME"))
        LICENSE_ID = vault.get("LICENSE_ID")
        NODE_ID = str(vault.get("NODE_ID") or "").strip() or None

        # Non-critical backend housekeeping starts asynchronously. Never join
        # these threads from the boot path: a slow bucket operation, database
        # reconciliation, DNS/TLS stall, or RLS response must not postpone
        # Realtime C2 or security subsystem startup.
        threading.Thread(target=ensure_bucket, daemon=True, name="ensure_bucket").start()
        threading.Thread(
            target=_background_registration_loop,
            daemon=True,
            name="register_workstation",
        ).start()

        # The online heartbeat is no longer part of the critical path. Local
        # cached license state is authoritative for immediate boot decisions; a
        # background check updates it and signals LICENSE_INVALID_EVENT if the
        # server reports revocation/suspension/expiry.
        boot_status = str(vault.get("LICENSE_STATUS") or "valid").lower()
        threading.Thread(target=_boot_license_watch, daemon=True, name="boot_license_check").start()

        if boot_status in ("revoked", "suspended", "expired"):
            def _hostile_watermark_thread(status):
                # We offload the actual drawing to the Rust core for native performance
                # and to avoid blocking the OS UI thread with Tkinter.
                logger.info("Signaling Rust Core to display Hostile Watermark.", component="system")
                try:
                    # Show the watermark overlay
                    _core_ipc_call({"cmd": "show_overlay", "kind": "watermark"})
                except Exception as e:
                    logger.error(f"Failed to signal Rust Core for watermark: {e}", component="system")
        ...
        # The Warden hook installation call is synchronous and has completed.
        # Do not add an arbitrary startup sleep here: the worker graph has its
        # own readiness/error handling and delaying main() only postpones
        # Realtime C2 and the scanner for no security benefit.
        # Launch the Agent
        main()
