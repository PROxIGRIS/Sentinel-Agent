...
- **2026-09 regression note:** `docs/engineering/howtoassemble.md` — a
  separate from-scratch build guide — had drifted to instruct
  `pyinstaller --onefile --noconsole --name obylon Obylon.py`, directly
  contradicting this document. An agent that read only that guide (it's the
  more complete end-to-end build walkthrough, so it's an easy one to reach
  for) would reintroduce this exact bug. It's been corrected to point back
  here, but if boot latency regresses again, check that file too — don't
  assume this being correct means every build doc agrees with it.
