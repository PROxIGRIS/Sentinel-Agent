...
**Packaging mode is not a free choice here — read `docs/agent/importantforagent/README.md`
("Issue 1: PyInstaller `--onefile` vs `--onedir` Boot Delay") before running
PyInstaller.** `--onefile` bundles the entire Python runtime into one exe that
self-extracts to a fresh `%LocalAppData%\Temp\_MEIxxxxxx` on *every single
launch* — that's the exact 10-30-second boot delay a prior pass diagnosed and
fixed once already. Building with `--onefile` (as an earlier revision of this
file incorrectly instructed) silently reintroduces that regression: the
installer still runs and even still installs (`obylon-setup.iss` looks for
`..\dist\obylon\*`, the onedir layout), but every boot pays the extraction
cost again. `obylon.spec` (gitignored, built locally) is the authoritative
config and MUST use `COLLECT()` — never pass `--onefile` on the command line
either, since that bypasses the spec entirely.

```
python -m py_compile Obylon.py    # syntax only — see the note below on why this isn't enough
python -m PyInstaller -y obylon.spec    # NOT `pyinstaller --onefile ...` — see above
```
