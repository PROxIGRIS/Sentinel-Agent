...
fn ensure_acls(logger: &FileLogger) {
    ...
    // SECURITY/RELIABILITY FIX (2026-09): this used to widen exactly
    // three files (obylon.enc, identity_beacon.json, fastlane_rules.json)
    // and nothing else. installer/obylon-setup.iss locks the whole
    // Obylon\ tree (including Obylon\logs\) to SYSTEM+Administrators
    // only, and Core/Brain run as the interactive user — usually a
    // non-admin student. Every other file the interactive-user
    // processes actually write (Core's own log, node identity, the
    // recovery journal, screenshot/webcam capture, the fast-lane
    // durable event queue) was left unwritable — see report.md finding
    // #3. FileLogger::open() now has its own fallback too (lib.rs), but
    // fixing the ACL is the real fix; the fallback is a safety net for
    // if this icacls call itself fails for some environmental reason.
    //
    // Directories get (OI)(CI) so files *created later* inside them (a
    // new day's core.log, each new capture/*.jpg, events.jsonl) inherit
    // the grant automatically instead of needing to be enumerated here.
    ...
    // Individual files that live directly under Obylon\ (not covered by
    // the directory inheritance above). node_binding.json / node_name /
    // .machine_id / recovery_journal.json are all written directly by
    // the interactive-user Brain process — see node_identity.py's
    // save_binding()/save_machine_name() and Obylon.py's
    // IDENTITY_FILE.write_text() / recovery-journal persistence.
    // .machine_id was previously commented as "student-read-only" in
    // harden_installation() while the identity-minting code in the same
    // file wrote to it directly as that same student — that
    // contradiction was the actual bug; this resolves it in favor of
    // the write path that's demonstrably exercised at runtime.
    // license_seed.txt is deliberately NOT in this list: it's only ever
    // read by the agent (zero-touch provisioning seed dropped by IT at
    // install time), never written by it.
    let files = [
        "obylon.enc",
