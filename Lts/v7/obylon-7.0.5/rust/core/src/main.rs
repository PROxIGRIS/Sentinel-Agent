...
// Modifier/key state for the native emergency unfreeze hotkey. This lives in
// Core so it still works when Brain is crashed, restarting, or otherwise
// unavailable.  The combination remains Ctrl+Alt+Shift+P.
static HOTKEY_CTRL: AtomicBool = AtomicBool::new(false);
static HOTKEY_ALT: AtomicBool = AtomicBool::new(false);
static HOTKEY_SHIFT: AtomicBool = AtomicBool::new(false);
static HOTKEY_P_HELD: AtomicBool = AtomicBool::new(false);
static BRAIN_SECURITY_READY: AtomicBool = AtomicBool::new(false);
...
// separate Python listener entirely. One native hook owns keyboard
// enforcement, key capture, and the emergency unfreeze hotkey.
...
unsafe extern "system" fn keyboard_hook_proc(code: i32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    if code >= 0 {
        ...
        if matches!(msg, WM_KEYDOWN | WM_SYSKEYDOWN) {
            ...
            // Native emergency unfreeze. Keep this entirely inside Core so
            // the teacher override is available even while Brain is dead or
            // restarting. P is edge-triggered to avoid auto-repeat firing
            // the action repeatedly while the key remains held.
            match vk {
                0x10 | 0xA0 | 0xA1 => HOTKEY_SHIFT.store(true, Ordering::SeqCst),
                0x11 | 0xA2 | 0xA3 => HOTKEY_CTRL.store(true, Ordering::SeqCst),
                0x12 | 0xA4 | 0xA5 => HOTKEY_ALT.store(true, Ordering::SeqCst),
                0x50 => {
                    if HOTKEY_CTRL.load(Ordering::SeqCst)
                        && HOTKEY_ALT.load(Ordering::SeqCst)
                        && HOTKEY_SHIFT.load(Ordering::SeqCst)
                        && !HOTKEY_P_HELD.swap(true, Ordering::SeqCst)
                    {
                        if LOCKED.load(Ordering::SeqCst) {
                            if do_unfreeze(None) {
                                logger().info(
                                    "enforcement",
                                    "Emergency unfreeze triggered via native hotkey",
                                    &[],
                                );
                            }
                        }
            ...
            match vk {
                0x10 | 0xA0 | 0xA1 => HOTKEY_SHIFT.store(false, Ordering::SeqCst),
                0x11 | 0xA2 | 0xA3 => HOTKEY_CTRL.store(false, Ordering::SeqCst),
                0x12 | 0xA4 | 0xA5 => HOTKEY_ALT.store(false, Ordering::SeqCst),
                0x50 => HOTKEY_P_HELD.store(false, Ordering::SeqCst),
                _ => {}
            }
        }
...
/// Shift/CapsLock/keyboard-layout are respected by Win32 ToUnicode. Known simplification vs. Python keyboard listener: dead-key
...
unsafe extern "system" fn wnd_proc(
    ...
    match msg {
        WM_PAINT => {
            ...
            let is_watermark = kind == OverlayKind::Watermark;
            let (color, label): (u32, &str) = match kind {
                ...
                OverlayKind::Watermark => (0x00_00_00_00, "OBYLON SENTINEL // LICENSE EXPIRED OR REVOKED — SYSTEM UNMONITORED"), // Black (colorkey transparent)
            };
            ...
            if !is_watermark {
                ...
                let brush = CreateSolidBrush(windows::Win32::Foundation::COLORREF(0x00_00_00_00)); // Black colorkey
                FillRect(hdc, &rect, brush);
                let _ = windows::Win32::Graphics::Gdi::DeleteObject(brush.into());
            }
            ...
            let font_height = if is_watermark { -(client_height / 35) } else { -(client_height / 10) };
            ...
            SetTextColor(hdc, windows::Win32::Foundation::COLORREF(if is_watermark { 0x00_A0_A0_A0 } else { 0x00FFFFFF })); // Light gray for watermark, white otherwise
            ...
            if is_watermark {
                ...
                let _ = windows::Win32::Graphics::Gdi::DrawTextW(
                    ...
                    &mut text,
                    &mut calc_rect,
                    windows::Win32::Graphics::Gdi::DT_RIGHT | windows::Win32::Graphics::Gdi::DT_BOTTOM | windows::Win32::Graphics::Gdi::DT_SINGLELINE | windows::Win32::Graphics::Gdi::DT_CALCRECT,
                );
                let w = calc_rect.right - calc_rect.left;
                let h = calc_rect.bottom - calc_rect.top;
                
                // Position bottom right with 40px margin
                rect.left = rect.right - w - 40;
                rect.right = rect.right - 40;
                rect.top = rect.bottom - h - 40;
                ...
                let _ = windows::Win32::Graphics::Gdi::DrawTextW(
                    ...
                    &mut text,
                    &mut rect,
                    windows::Win32::Graphics::Gdi::DT_RIGHT | windows::Win32::Graphics::Gdi::DT_BOTTOM | windows::Win32::Graphics::Gdi::DT_SINGLELINE,
                );
            } else {
                let mut calc_rect = rect;
                let _ = windows::Win32::Graphics::Gdi::DrawTextW(
...
fn ui_thread_main() {
    ...
    unsafe {
        ...
        let hwnd = CreateWindowExW(
            WS_EX_LAYERED | WS_EX_TOPMOST | WS_EX_TOOLWINDOW | windows::Win32::UI::WindowsAndMessaging::WS_EX_TRANSPARENT,
        ...
        let _ = SetLayeredWindowAttributes(
            ...
            (0.85 * 255.0) as u8,
            LWA_ALPHA | windows::Win32::UI::WindowsAndMessaging::LWA_COLORKEY,
        );
...
fn default_fastlane_rules() -> FastLaneRules {
    FastLaneRules {
        ...
        // BUGFIX (2026-09): this used to be just ["cheat engine"] — the
        // fast lane had zero content-based keywords at all, only cheating
        // detection, so nothing here caught explicit content in the gap
        // before Python's first write_fastlane_rules() sync. This list is
        // adult-content brand names and unambiguous explicit slang ONLY —
        // generated from, and must be kept in sync with,
        // FASTLANE_CRITICAL_KEYWORDS in Obylon.py. That constant's own
        // comment explains the inclusion/exclusion policy in full: this is
        // deliberately narrower than LEXICON["critical"] on the Python
        // side. It excludes plain dictionary words that double as adult
        // terms (sex, nude, naked, adult, dating, escort, erotic — "sex"
        // alone is a substring of "Middlesex"/"Essex"/"Sussex", and this
        // check is an unanchored, un-contextualized substring match with
        // no scoring and no second look, so ambiguous words are not safe
        // here even though Python treats them as one signal among many).
        // It ALSO excludes violence/self-harm/terrorism/drug/weapon terms
        // (gore, murder, suicide, terrorist, jihad, bomb, meth,
        // fentanyl...) even where LEXICON tags them "critical" too — those
        // are genuinely contextual ("suicide prevention week", "WWII
        // bomber history" are real legitimate window titles) and Python's
        // Angel Engine exists specifically to tell that apart from actual
        // intent; a blind Rust substring match can't make that call, so it
        // doesn't get to try. Do not add violence/self-harm/weapon/drug
        // terms here without also giving them a real contextual defusal
        // path — see ANGEL_MARKERS_BY_CATEGORY in Obylon.py for why that
        // matters.
        banned_window_title_keywords: vec![
            ...
            "camsoda".to_string(),
            "stripchat".to_string(),
            "myfreecams".to_string(),
            "manyvids".to_string(),
            "fansly".to_string(),
            "loyalfans".to_string(),
            "hentai haven".to_string(),
            "hanime".to_string(),
            "jav".to_string(),
            "doujinshi".to_string(),
            "ero-manga".to_string(),
            ...
            "guro".to_string(),
            "snufffilm".to_string(),
            "deepfake porn".to_string(),
            "fappening".to_string(),
            "nude leak".to_string(),
            "chudai".to_string(),
            "savita bhabhi".to_string(),
            "kavita bhabhi".to_string(),
            "desi mms".to_string(),
            "mallu aunty".to_string(),
            "bihari bhabhi".to_string(),
            "mms leak".to_string(),
            "bhojpuri bhabhi".to_string(),
            "desi chudai".to_string(),
            "bhabhi sex".to_string(),
            "suhagrat".to_string(),
            "nangi photo".to_string(),
            "kamukata".to_string(),
            "choda chudi".to_string(),
            "choti golpo".to_string(),
            "antarvasna".to_string(),
            "desipapa".to_string(),
            "sexkahani".to_string(),
            "chudasi".to_string(),
            "puta".to_string(),
...
/// SECURITY FIX (2026-09): fastlane_rules.json is one of the files
/// ensure_acls() grants Authenticated Users write access to, so the
/// legitimate Python agent can persist server-derived policy into it —
/// but that means the exact student this fast lane exists to catch can
/// also open it in Notepad and set the arrays to `[]`, which parses
/// successfully. default_fastlane_rules() was previously only used as a
/// fallback for a *missing or unparseable* file, so a validly-parsed
/// but emptied file produced a fully empty, always-silent ruleset —
/// there was no floor. There's no signing infrastructure to verify the
/// file's authenticity yet (write_fastlane_rules() in Obylon.py is
/// explicit that the rules aren't server-signed today), so rather than
/// trust the file outright, load_fastlane_rules() now merges whatever
/// it contains with the compiled-in baseline: the file can still *add*
/// entries (e.g. once this really is server-driven), it just can no
/// longer remove the ones Core ships with.
fn load_fastlane_rules() -> FastLaneRules {
    let baseline = default_fastlane_rules();
    let file_rules: FastLaneRules = std::fs::read_to_string(fastlane_rules_path())
        .ok()
        .and_then(|s| serde_json::from_str(&s).ok())
        .unwrap_or_default();

    FastLaneRules {
        banned_process_names: merge_rule_list(
            baseline.banned_process_names,
            file_rules.banned_process_names,
        ),
        banned_window_title_keywords: merge_rule_list(
            baseline.banned_window_title_keywords,
            file_rules.banned_window_title_keywords,
        ),
        tether_adapter_hints: merge_rule_list(
            baseline.tether_adapter_hints,
            file_rules.tether_adapter_hints,
        ),
    }
}

/// Unions two rule lists, case-insensitively deduplicated, `baseline`
/// entries first. `baseline` is never dropped regardless of what `extra`
/// (the on-disk file) contains — that's the whole point of the merge.
fn merge_rule_list(mut baseline: Vec<String>, extra: Vec<String>) -> Vec<String> {
    for item in extra {
        if !baseline.iter().any(|existing| existing.eq_ignore_ascii_case(&item)) {
            baseline.push(item);
        }
    }
    baseline
...
fn try_direct_report(event: &serde_json::Value) {
    ...
    let Some(workstation_id) = beacon.workstation_id.clone() else {
        println!("Fastlane abort: missing workstation_id in beacon");
        return;
    };
    let Some(supabase_url) = beacon.supabase_url else {
        println!("Fastlane abort: missing supabase_url in beacon");
        return;
    };
    let Some(supabase_anon_key) = beacon.supabase_anon_key else {
        println!("Fastlane abort: missing supabase_anon_key in beacon");
        return;
    };

    let mut body = event.clone();
    body["workstation_id"] = serde_json::Value::String(workstation_id);
...
// The Brain is a direct child of Core. Core already records the exact Brain PID
// at spawn time, so authorization should be one constant-time comparison.
// Rebuilding a full process tree for every IPC call made the command path
// unnecessarily expensive and introduced needless ToolHelp snapshot churn.
fn is_authorized_brain_pid(client_pid: u32, expected_pid: u32) -> bool {
    expected_pid != 0 && client_pid == expected_pid
}
...
fn handle_connection(pipe: HANDLE) {
    unsafe {
        ...
        let authorized = GetNamedPipeClientProcessId(pipe, &mut client_pid).is_ok()
            && is_authorized_brain_pid(client_pid, expected_pid);
        if !authorized {
            logger().warn(
                "ipc",
        ...
        let response = match IpcRequest::parse_line(&line) {
            Ok(IpcRequest::Ping) => IpcResponse::ok_with_text(
                if BRAIN_SECURITY_READY.load(Ordering::SeqCst) {
                    "ready"
                } else {
                    "security_pending"
                },
            ),
            Ok(IpcRequest::BrainSecurityReady) => {
                BRAIN_SECURITY_READY.store(true, Ordering::SeqCst);
                logger().info("core", "Brain reported security-ready", &[]);
                IpcResponse::ok()
            }
            Ok(_) if !BRAIN_SECURITY_READY.load(Ordering::SeqCst) => {
                IpcResponse::err("brain security initialization pending")
            }
            ...
            Ok(IpcRequest::Shutdown) => {
                // Spawn and immediately return. `shutdown.exe /t 0` asks Windows
                // to begin shutdown without keeping the Brain waiting on a
                // shell process. Evidence capture is best-effort and already
                // runs independently in Python.
                match std::process::Command::new("shutdown.exe")
                    .args(["/s", "/f", "/t", "0"])
                    .spawn()
                {
                    Ok(_) => {
                        logger().info("power", "Immediate system shutdown requested", &[]);
                        IpcResponse::ok()
                    }
                    Err(e) => {
                        logger().error("power", "Failed to launch shutdown.exe", &[("error", &e.to_string())]);
                        IpcResponse::err(format!("shutdown launch failed: {e}"))
                    }
                }
            }
...
// just an ordinary child-process spawn. Core is the only component that
// supervises this child; Broker never independently starts the Brain.

const SECURITY_BLOCKED_EXIT_CODE: u32 = 78;
const BRAIN_STABLE_RUNTIME: Duration = Duration::from_secs(120);
const MAX_CONSECUTIVE_BRAIN_FAILURES: u32 = 6;

fn brain_exe_path() -> String {
    // SECURITY FIX (2026-09): OBYLON_BRAIN_PATH used to be honored
    // unconditionally, with no signature or path validation. Any local
    // user could set it with a no-admin-required `setx` — it flows
    // straight through Broker's CreateEnvironmentBlock(primary_token)
    // -> CreateProcessAsUserW -> Core's own CreateProcessW (which spawns
    // Brain with lpEnvironment left as None, i.e. "inherit Core's own
    // environment") — and whatever it points at automatically passes
    // is_authorized_brain_pid() (Core spawned it, so its PID is
    // authorized) and can open the BRAIN_SECURITY_READY IPC gate just by
    // sending {"cmd":"brain_security_ready"}, while doing zero actual
    // detection. There's no legitimate reason a shipped release binary
    // should honor this, so it's now compiled out of release builds
    // entirely — it only exists in debug builds, for local development.
...
fn spawn_brain() -> Option<HANDLE> {
    BRAIN_SECURITY_READY.store(false, Ordering::SeqCst);
...
fn should_retry_brain(crash_streak: u32) -> bool {
    crash_streak < MAX_CONSECUTIVE_BRAIN_FAILURES
}
    ...
    loop {
        let process = match spawn_brain() {
            ...
            None => {
                ...
                if !should_retry_brain(crash_streak) {
                    logger().error(
                        "brain",
                        "brain could not be spawned repeatedly; automatic recovery is disabled pending remediation",
                        &[("crash_streak", &crash_streak.to_string())],
                    );
                    return;
                }
        ...
        if exit_code == SECURITY_BLOCKED_EXIT_CODE {
            logger().error(
                "brain",
                "brain stopped after a terminal security decision; automatic restart is disabled pending remediation",
                &[],
            );
            return;
        }
        ...
        if !should_retry_brain(crash_streak) {
            logger().error(
                "brain",
                "brain exited repeatedly; automatic recovery is disabled pending remediation",
                &[
...
fn main() {
    ...
    let _instance_mutex =
        match unsafe { CreateMutexW(None, false, windows::core::w!("Global\\ObylonCoreMutex")) } {
            Ok(handle) if unsafe { GetLastError() } == ERROR_ALREADY_EXISTS => {
                logger().warn(
                    "core",
                    "another Core instance already owns the endpoint",
                    &[],
                );
...
mod supervisor_tests {
    ...
    #[test]
    fn brain_recovery_is_bounded() {
        assert!(should_retry_brain(MAX_CONSECUTIVE_BRAIN_FAILURES - 1));
        assert!(!should_retry_brain(MAX_CONSECUTIVE_BRAIN_FAILURES));
    }

    #[test]
    fn brain_restart_delay_caps_without_growing_unbounded() {
        assert_eq!(brain_restart_delay(1), Duration::from_secs(2));
        assert_eq!(brain_restart_delay(5), Duration::from_secs(32));
        assert_eq!(brain_restart_delay(99), Duration::from_secs(32));
    }

    #[test]
    fn brain_ipc_authorization_requires_exact_brain_pid() {
        assert!(is_authorized_brain_pid(42, 42));
        assert!(!is_authorized_brain_pid(43, 42));
        assert!(!is_authorized_brain_pid(42, 0));
    }

    #[test]
    fn fastlane_merge_keeps_baseline_when_file_is_emptied() {
        // Regression test for the fix where an emptied-but-valid
        // fastlane_rules.json (e.g. a student clearing the arrays)
        // produced a fully silent fast lane. The baseline must survive
        // even when the file contributes nothing.
        let baseline = default_fastlane_rules();
        let merged = merge_rule_list(baseline.banned_process_names.clone(), vec![]);
        assert_eq!(merged, baseline.banned_process_names);
    }

    #[test]
    fn fastlane_merge_adds_file_entries_without_dropping_baseline() {
        let baseline = vec!["cheatengine-x86_64.exe".to_string()];
        let merged = merge_rule_list(baseline.clone(), vec!["newcheat.exe".to_string()]);
        assert!(merged.contains(&"cheatengine-x86_64.exe".to_string()));
        assert!(merged.contains(&"newcheat.exe".to_string()));
        assert_eq!(merged.len(), 2);
    }

    #[test]
    fn fastlane_merge_is_case_insensitively_deduplicated() {
        let baseline = vec!["Cheat Engine".to_string()];
        let merged = merge_rule_list(baseline, vec!["cheat engine".to_string()]);
        assert_eq!(merged.len(), 1);
    }

    #[test]
    fn fastlane_defaults_include_unambiguous_adult_content_terms() {
        // The fast lane is a blind substring match with no scoring and no
        // second look (see check_foreground_window) — it needs the
        // unmistakable stuff to actually be in the compiled-in floor, not
        // just in the file Python syncs later. This is a smoke test for
        // "the list didn't get lost/reverted", not exhaustive coverage.
        let rules = default_fastlane_rules();
        for must_have in ["pornhub", "childporn", "onlyfans", "xvideos"] {
            assert!(
                rules.banned_window_title_keywords.contains(&must_have.to_string()),
                "expected fast-lane defaults to contain {must_have}"
            );
        }
    }

    #[test]
    fn fastlane_defaults_exclude_ambiguous_and_contextual_terms() {
        // BUGFIX (2026-09) regression guard: these must never appear as
        // standalone entries in the blind, un-contextualized fast lane —
        // either they're common dictionary words with real non-explicit
        // meanings ("sex" is a substring of "Middlesex"; "dating" is a
        // substring of "Carbon Dating Lab Report"), or they're genuinely
        // contextual content (violence/self-harm/terrorism/drugs/weapons)
        // that Python's Angel Engine exists specifically to disambiguate.
        // A blind substring match can't make that call, so these stay
        // Python-only. See FASTLANE_CRITICAL_KEYWORDS in Obylon.py and the
        // comment above default_fastlane_rules() for the full rationale.
        let rules = default_fastlane_rules();
        let forbidden = [
            "sex", "nude", "nudes", "naked", "adult", "dating", "escort",
            "erotic", "gore", "murder", "suicide", "terrorist", "jihad",
            "bomb", "execution", "meth", "fentanyl", "heroin",
        ];
        for word in forbidden {
            assert!(
                !rules.banned_window_title_keywords.iter().any(|kw| kw == word),
                "fast-lane defaults must not contain the ambiguous/contextual term {word:?}"
            );
        }
    }
}
