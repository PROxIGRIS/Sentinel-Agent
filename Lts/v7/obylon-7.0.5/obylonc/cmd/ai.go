...
- obylonc deactivate [-y]: Deactivates the endpoint; privileged.
- obylonc admin <command>: Explicit privileged command namespace.
- obylonc auth <command>: Umbraxis authorization plus admin shortcuts such as auth deactivate.

Activation policy:
License activation is performed by the installer using the obylonc activate provisioning path. Manual activation remains available for recovery and re-provisioning.
The activation request sends the workstation name in node_name, device_name, and hostname fields.
