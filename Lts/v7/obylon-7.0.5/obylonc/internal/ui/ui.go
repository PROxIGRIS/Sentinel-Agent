...
// Semantic status helpers used by the command UI.
func StatusOK(s string) string      { return Green("● " + s) }
func StatusBad(s string) string     { return Red("● " + s) }
func StatusWarn(s string) string    { return Yellow("● " + s) }
func StatusNeutral(s string) string { return Dim("● " + s) }
...
func PrintBanner(byline string) {
	...
	fmt.Println(Dim("                   " + "Umbraxis / Obylon Sentinel"))
	fmt.Println()
}
...
func PrintCompactHeader(title, subtitle string) {
	...
	fmt.Println(Dim("   " + strings.Repeat("─", 54)))
}
