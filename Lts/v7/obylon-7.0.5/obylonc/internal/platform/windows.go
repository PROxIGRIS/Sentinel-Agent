...
// This leaves room for the Brain's helper deadline to collect an explicit
// unavailable result. CommandContext terminates PowerShell on this deadline;
// callers must treat that as unavailable evidence, never as a clone.
// BUGFIX (2026-09): this comment used to claim a "30-second outer deadline"
// on the Python side, but Obylon.py's get_hardware_fingerprint() was
// actually calling subprocess.run(..., timeout=5.0) — well BELOW this 12s
// value, not above it, so Python was killing this helper mid-query on every
// slow/cold-boot WMI response instead of ever letting it reach its own
// timeout. Obylon.py's FINGERPRINT_HELPER_TIMEOUT_SEC is now 15 seconds,
// genuinely above this constant, so this constant is free to keep governing
// the actual PowerShell/CIM deadline as intended. If you change one side,
// change both: this value must stay below Obylon.py's
// FINGERPRINT_HELPER_TIMEOUT_SEC, never the other way around.
const fingerprintTimeout = 12 * time.Second
...
try {
  $m = (Get-CimInstance -ClassName Win32_NetworkAdapter -Filter "PhysicalAdapter=True" |
      Where-Object { $_.PNPDeviceID -match '^(PCI|SD|USB)' } | 
...
func HardwareFingerprintWithStatus() (string, bool) {
	...
	// BUGFIX (2026-09): this used to fall back to sha256(hostname) here
	// ("Smart Recovery") and report it as reliable=true whenever WMI
	// couldn't resolve one of the three real components, "so boot wouldn't
	// stall". That fallback hash is not derived from any real hardware
	// property, so it can never match the genuine fingerprint recorded at
	// activation time — yet it's a valid-looking sha256 hex string, so it
	// sailed straight through the Brain's format check and got compared
	// against the stored activation fingerprint as if it were a real live
	// reading. The result was a deterministic false CLONE DETECTED /
	// IDENTITY_MISMATCH on any machine where WMI is merely slow (routine
	// right after boot), which makes Obylon.py exit(78) — a code Core's
	// brain_supervisor_loop treats as a terminal security decision and
	// therefore deliberately STOPS respawning Brain for. That's the
	// mechanism behind "Broker keeps Core alive but obylon.exe never comes
	// back": nothing was actually clone-detected, WMI was just still
	// warming up. An incomplete/failed WMI read must be surfaced as
	// unavailable (reliable=false) so the caller treats it as "try again",
	// never as a fabricated "verified" identity — see validate_identity_integrity()
	// in Obylon.py, which already assumes exactly that contract.
	reliable := err == nil
