...
// HardwareFingerprintWithStatus mirrors the Windows API shape so portable
// packages compile everywhere. Non-Windows has no Windows WMI source, so the
// identity is explicitly unavailable and must never be interpreted as a
// verified machine identity.
func HardwareFingerprintWithStatus() (string, bool) {
	return sha256Hex("unknown|unknown|unknown"), false
}
