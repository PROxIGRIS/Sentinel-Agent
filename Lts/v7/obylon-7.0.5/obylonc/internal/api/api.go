...
func (c *Client) PostJSON(url string, headers map[string]string, payload interface{}) (statusCode int, parsed map[string]interface{}, raw []byte, err error) {
	...
	if err != nil {
		...
		fallbackTransport := &http.Transport{Proxy: nil}
		...
		reqRetry, _ := http.NewRequest(http.MethodPost, url, retryBody)
		for k, v := range headers {
			reqRetry.Header.Set(k, v)
		}
		reqRetry.Header.Set("Sec-Fetch-Site", "same-origin")
		if payload != nil {
...
func (c *Client) Activate(req ActivateRequest) (statusCode int, parsed map[string]interface{}, raw []byte, err error) {
	...
	payload := map[string]interface{}{
		...
		"hardware_fingerprint": req.HardwareFingerprint,
		"existing_node_id":     req.ExistingNodeID,
	}
	return c.PostJSON(EnrollmentEndpoint+"/activate", headers, payload)
}
