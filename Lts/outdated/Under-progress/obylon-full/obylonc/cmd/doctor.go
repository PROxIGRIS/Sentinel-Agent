...
func checkBootTask() (ok bool, message string) {
	out, err := exec.Command("schtasks", "/query", "/tn", "ObylonAgent", "/v", "/fo", "list").CombinedOutput()
	if err != nil {
	...
	if !strings.Contains(strings.ToLower(text), "obylonbroker.exe") {
		return false, "boot task exists but does not point at ObylonBroker.exe (stale target from before the Rust broker split?)"
	}
	return true, "boot task is registered, enabled, and targets ObylonBroker.exe"
}

func fixBootTask() error {
	appPath := paths.DefaultBrokerExePath()
	out, err := exec.Command("schtasks", "/create", "/tn", "ObylonAgent",
