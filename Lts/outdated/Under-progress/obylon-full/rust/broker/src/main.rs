...
use windows::Win32::Foundation::{CloseHandle, GetLastError, HANDLE, HWND, LUID};
use windows::Win32::Security::{
    AdjustTokenPrivileges, DuplicateTokenEx, LookupPrivilegeValueW, SecurityImpersonation,
    TokenPrimary, LUID_AND_ATTRIBUTES, SE_DEBUG_NAME, SE_INCREASE_QUOTA_NAME,
    SE_PRIVILEGE_ENABLED, SE_TCB_NAME, TOKEN_ADJUST_PRIVILEGES, TOKEN_ALL_ACCESS, TOKEN_PRIVILEGES,
    TOKEN_QUERY,
};
...
use windows::Win32::System::Threading::{
    CreateProcessAsUserW, GetCurrentProcess, OpenProcess, OpenProcessToken, CREATE_NO_WINDOW,
...
fn log_path() -> PathBuf {
    ...
    PathBuf::from(base).join("Obylon").join("logs").join("broker.log")
}
...
unsafe fn enable_privileges(logger: &FileLogger) {
    ...
    .is_err()
    {
        logger.error("broker", "OpenProcessToken failed", &[("error", &format!("{:?}", GetLastError()))]);
        return;
    }
    ...
    for priv_name in [SE_TCB_NAME, SE_INCREASE_QUOTA_NAME, SE_DEBUG_NAME] {
        ...
        if LookupPrivilegeValueW(PCWSTR::null(), priv_name, &mut luid).is_err() {
            logger.warn("broker", "LookupPrivilegeValueW failed", &[("priv", &format!("{:?}", priv_name))]);
            continue;
        }
...
unsafe fn borrow_shell_token(session_id: u32, logger: &FileLogger) -> Option<HANDLE> {
    ...
    if Process32FirstW(snapshot, &mut entry).is_ok() {
        loop {
            ...
            if CANDIDATE_SHELLS.contains(&name.as_str()) {
                ...
                {
                    if let Ok(ph) = OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, false, entry.th32ProcessID) {
    ...
    if found.is_none() {
        logger.warn("broker", "No shell token to borrow in target session", &[("session_id", &session_id.to_string())]);
    }
    found
}
...
unsafe fn get_session_token(session_id: u32, logger: &FileLogger) -> Option<(HANDLE, &'static str)> {
...
unsafe fn spawn_core_in_session(session_id: u32, core_exe: &str, logger: &FileLogger) -> Option<u32> {
    ...
    if dup_ok.is_err() {
        logger.error("broker", "DuplicateTokenEx failed", &[("error", &format!("{:?}", GetLastError()))]);
        return None;
    }
    ...
    if ok.is_err() {
        logger.error(
            ...
            &[("session_id", &session_id.to_string()), ("error", &format!("{:?}", GetLastError()))],
        );
        return None;
    }
    ...
    logger.info(
        ...
        &[("session_id", &session_id.to_string()), ("pid", &pid.to_string()), ("token_kind", token_kind)],
    );
    Some(pid)
}
...
fn main() {
    let logger = match FileLogger::open(&log_path()) {
        Ok(l) => l,
        Err(_) => FileLogger::open(&PathBuf::from(LOG_PATH_ENV_FALLBACK)).expect("cannot open any broker log path"),
    };
    logger.info("broker", "Session Broker online — waiting for an interactive console session", &[]);
