...
impl IpcResponse {
    pub fn ok() -> Self {
        Self { ok: true, error: None, locked: None, text: None, path: None }
    }
    pub fn ok_with_locked(locked: bool) -> Self {
        Self { ok: true, error: None, locked: Some(locked), text: None, path: None }
    }
    pub fn ok_with_text(text: impl Into<String>) -> Self {
        Self { ok: true, error: None, locked: None, text: Some(text.into()), path: None }
    }
    pub fn ok_with_path(path: impl Into<String>) -> Self {
        Self { ok: true, error: None, locked: None, text: None, path: Some(path.into()) }
    }
    pub fn err(msg: impl Into<String>) -> Self {
        Self { ok: false, error: Some(msg.into()), locked: None, text: None, path: None }
    }
...
mod tests {
    ...
    #[test]
    fn freeze_reason_defaults_to_classroom_focus_when_omitted() {
        // Guards backward compatibility with any caller that predates the
        // `reason` field (verified bug #2/#3 fix).
        let json = r#"{"cmd":"freeze","duration_secs":30}"#;
        let req = IpcRequest::parse_line(json).unwrap();
        match req {
            IpcRequest::Freeze { reason, .. } => assert_eq!(reason, OverlayKind::ClassroomFocus),
            _ => panic!("wrong variant"),
        }
    }
    ...
    fn unfreeze_with_classroom_focus_reason_round_trips() {
        ...
        let req = IpcRequest::parse_line(json).unwrap();
        match req {
            IpcRequest::Unfreeze { reason } => assert_eq!(reason, Some(OverlayKind::ClassroomFocus)),
            _ => panic!("wrong variant"),
        }
    }
    ...
    fn keylog_and_capture_requests_round_trip() {
        for shape in [
            ...
            assert!(IpcRequest::parse_line(shape).is_ok(), "failed to parse: {shape}");
        }
    }
