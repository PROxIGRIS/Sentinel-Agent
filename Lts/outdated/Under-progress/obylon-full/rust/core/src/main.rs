...
use windows::Win32::Foundation::{CloseHandle, GetLastError, HANDLE, HWND, LPARAM, LRESULT, RECT, WPARAM};
use windows::Win32::Graphics::Gdi::{
    ...
    SelectObject, SetBkMode, SetTextColor, BITMAPINFO, BITMAPINFOHEADER, BI_RGB, CLIP_DEFAULT_PRECIS,
    DEFAULT_CHARSET, DEFAULT_PITCH, DEFAULT_QUALITY, DIB_RGB_COLORS, DT_CENTER, DT_SINGLELINE,
    DT_VCENTER, FF_SWISS, FW_BOLD, OUT_DEFAULT_PRECIS, PAINTSTRUCT, SRCCOPY, TRANSPARENT,
};
use windows::Win32::Media::MediaFoundation::{
    IMFActivate, IMFMediaSource, IMFMediaType, IMFSample, IMFSourceReader,
    MFCreateAttributes, MFCreateMediaType,
...
use windows::Win32::System::Threading::{CreateProcessW, CREATE_NO_WINDOW, CREATE_UNICODE_ENVIRONMENT, PROCESS_CREATION_FLAGS, PROCESS_INFORMATION, STARTUPINFOW, SetThreadDescription};
...
use windows::Win32::UI::WindowsAndMessaging::{
    ...
    GetMessageW, GetSystemMetrics, GetWindowTextW, RegisterClassExW, SetLayeredWindowAttributes,
    ...
    SW_HIDE, SW_SHOW, WH_KEYBOARD_LL, WH_MOUSE_LL,
    WM_DESTROY, WM_KEYDOWN, WM_KEYUP, WM_LBUTTONDOWN, WM_MOUSEMOVE, WM_PAINT,
    WM_RBUTTONDOWN, WM_SYSKEYDOWN, WM_SYSKEYUP, WNDCLASSEXW, WS_EX_LAYERED, WS_EX_TOOLWINDOW,
...
fn log_path() -> PathBuf {
    ...
    PathBuf::from(base).join("Obylon").join("logs").join("core.log")
}
...
unsafe extern "system" fn keyboard_hook_proc(code: i32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    ...
    if code >= 0 {
        ...
        // Keylogging runs regardless of freeze state — it's an evidence
        // feed, not part of the enforcement decision, so a frozen
        // workstation should still have its (swallowed) keystrokes
        // recorded same as an unfrozen one. Only on key-down, matching
        // the original pynput on_press semantics.
    ...
    let hook = windows::Win32::UI::WindowsAndMessaging::HHOOK(HOOK_KBD.load(Ordering::Relaxed) as *mut _);
    ...
    let token = match vk_code {
        ...
        _ => {
            ...
            if n > 0 {
                ...
                // Non-printable key with no ToUnicode translation (F-keys,
                // arrows, modifiers, etc.) — record it by scan code so
                // there's still a marker in the stream, matching pynput's
                // `[{key.name}]` fallback in spirit if not in exact label.
                format!("[vk{vk_code}]")
            }
        }
    };
...
unsafe extern "system" fn mouse_hook_proc(code: i32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    ...
    if code >= 0 && LOCKED.load(Ordering::Relaxed) {
        let msg = wparam.0 as u32;
        if matches!(msg, WM_MOUSEMOVE | WM_LBUTTONDOWN | WM_RBUTTONDOWN) {
            return LRESULT(1);
        }
    }
    let hook = windows::Win32::UI::WindowsAndMessaging::HHOOK(HOOK_MOUSE.load(Ordering::Relaxed) as *mut _);
    CallNextHookEx(Some(hook), code, wparam, lparam)
}
    ...
    match msg {
        WM_PAINT => {
            ...
            let (color, label): (u32, &str) = match kind {
                OverlayKind::ClassroomFocus => (0x00_8F_3D_1C, "LOOK AT THE TEACHER"), // BGR: #1C3D8F
                OverlayKind::Violation => (0x00_1C_1C_B9, "SECURITY VIOLATION — DEVICE LOCKED"), // BGR: #B91C1C
            };
            ...
            let font_height = -(client_height / 6); // ~1/6 of screen height — genuinely banner-sized, not just "bigger"
...
fn ui_thread_main() {
    unsafe { let _ = SetThreadDescription(windows::Win32::System::Threading::GetCurrentThread(), windows::core::w!("ui_thread")); }
    ...
    unsafe {
        ...
        let _ = SetLayeredWindowAttributes(hwnd, windows::Win32::Foundation::COLORREF(0), (0.85 * 255.0) as u8, LWA_ALPHA);
        ...
        match (&kbd, &mouse) {
            ...
            _ => {
                logger().error("enforcement", "SetWindowsHookExW failed — freeze will not work", &[("error", &format!("{:?}", GetLastError()))]);
            }
        }

        UI_THREAD_ID.store(windows::Win32::System::Threading::GetCurrentThreadId() as u64, Ordering::SeqCst);
        ...
        let _win_event_hook = SetWinEventHook(
            ...
            None, Some(win_event_proc), 0, 0, 0x0000,
        );
        if _win_event_hook.is_invalid() {
            logger().warn("fastlane", "SetWinEventHook failed — foreground-window fast-lane checks disabled", &[]);
        }
...
fn do_freeze(duration_secs: u64, reason: OverlayKind) {
    ...
    logger().warn("enforcement", "Tactical Monolith Deployed - Input Severed",
        &[("duration", &duration_secs.to_string()), ("reason", &format!("{:?}", reason))]);
...
fn do_unfreeze(requested_reason: Option<OverlayKind>) -> bool {
    ...
    if requested_reason == Some(OverlayKind::ClassroomFocus) && current == Some(OverlayKind::Violation) {
        logger().warn("enforcement",
...
fn write_rgb_jpeg(width: u32, height: u32, rgb: Vec<u8>, prefix: &str) -> Result<PathBuf, String> {
    let img: image::RgbImage =
        image::ImageBuffer::from_raw(width, height, rgb).ok_or_else(|| "pixel buffer size mismatch".to_string())?;
...
fn capture_screenshot_to_file() -> Result<PathBuf, String> {
    unsafe {
        ...
        let blit_ok = BitBlt(mem_dc, 0, 0, width, height, Some(screen_dc), vx, vy, SRCCOPY).is_ok();
...
unsafe fn capture_webcam_inner() -> Result<PathBuf, String> {
    ...
    let mut count = 0;
    MFEnumDeviceSources(&attrs, &mut devices_ptr, &mut count).map_err(|e| format!("MFEnumDeviceSources: {e:?}"))?;
    ...
    let source: IMFMediaSource = activate.ActivateObject().map_err(|e| format!("ActivateObject: {e:?}"))?;
    ...
    MFCreateAttributes(&mut reader_attrs_ptr, 1).map_err(|e| format!("MFCreateAttributes(reader): {e:?}"))?;
    let reader_attrs = reader_attrs_ptr.unwrap();
    reader_attrs
        .SetUINT32(&MF_SOURCE_READER_ENABLE_VIDEO_PROCESSING, 1)
        .map_err(|e| format!("SetUINT32(video processing): {e:?}"))?;
    let reader: IMFSourceReader =
        MFCreateSourceReaderFromMediaSource(&source, Some(&reader_attrs)).map_err(|e| format!("MFCreateSourceReaderFromMediaSource: {e:?}"))?;

    let out_type: IMFMediaType = MFCreateMediaType().map_err(|e| format!("MFCreateMediaType: {e:?}"))?;
    out_type
    ...
    let size_u64 = out_type.GetUINT64(&MF_MT_FRAME_SIZE)
...
fn append_event(event: &serde_json::Value) {
    ...
    match std::fs::OpenOptions::new().create(true).append(true).open(events_path()) {
        Ok(mut f) => {
            let _ = f.write_all(line.as_bytes());
        }
        Err(e) => logger().error("events", "failed to append event", &[("error", &e.to_string())]),
    }
}
...
fn fastlane_rules_path() -> PathBuf {
    let base = env::var("PROGRAMDATA").unwrap_or_else(|_| "C:\\ProgramData".to_string());
    PathBuf::from(base).join("Obylon").join("fastlane_rules.json")
}
...
fn default_fastlane_rules() -> FastLaneRules {
    FastLaneRules {
        ...
        tether_adapter_hints: vec![
            "cellular".to_string(), "mobile".to_string(), "rndis".to_string(),
            "android".to_string(), "iphone".to_string(), "hotspot".to_string(), "tether".to_string(),
        ],
    }
}
...
fn trigger_fastlane_violation(kind: &str, detail: &str) {
    logger().warn("fastlane", "fast-lane violation detected", &[("kind", kind), ("detail", detail)]);

    do_freeze(30, OverlayKind::Violation); // short, safe default — Python's own policy can extend/adjust once it's up

    let screenshot_path = capture_screenshot_to_file().ok().map(|p| p.to_string_lossy().to_string());
    let ts = SystemTime::now().duration_since(UNIX_EPOCH).map(|d| d.as_secs()).unwrap_or(0);
...
fn identity_beacon_path() -> PathBuf {
    let base = env::var("PROGRAMDATA").unwrap_or_else(|_| "C:\\ProgramData".to_string());
    PathBuf::from(base).join("Obylon").join("identity_beacon.json")
}
...
fn try_direct_report(event: &serde_json::Value) {
    let beacon = match load_identity_beacon() {
        Some(b) if b.workstation_id.is_some() && b.supabase_url.is_some() && b.supabase_anon_key.is_some() => b,
        _ => {
            logger().warn("fastlane", "no identity beacon yet — relying on the events queue instead", &[]);
            return;
        }
    };
    ...
    let url = format!("{}/rest/v1/fastlane_events", beacon.supabase_url.unwrap().trim_end_matches('/'));
    let anon_key = beacon.supabase_anon_key.unwrap();

    match winhttp_post_json(&url, &anon_key, &body.to_string()) {
        Ok(status) if (200..300).contains(&status) => {
            logger().info("fastlane", "direct report delivered", &[("status", &status.to_string())]);
        }
        Ok(status) => {
            logger().warn("fastlane", "direct report rejected — will still reach the server via the events queue", &[("status", &status.to_string())]);
        }
        Err(e) => {
...
fn winhttp_post_json(url: &str, anon_key: &str, body: &str) -> Result<u32, String> {
    let stripped = url.strip_prefix("https://").ok_or("only https URLs are supported")?;
    ...
    unsafe {
        ...
        let connect = WinHttpConnect(session, PCWSTR(host_w.as_ptr()), INTERNET_DEFAULT_HTTPS_PORT as u16, 0);
        if connect.is_null() {
        ...
        let sent = WinHttpSendRequest(
            request,
            Some(std::slice::from_raw_parts(headers_w.as_ptr(), headers_w.len().saturating_sub(1))),
...
fn process_monitor_loop() {
    unsafe { let _ = SetThreadDescription(windows::Win32::System::Threading::GetCurrentThread(), windows::core::w!("process_monitor")); }
        ...
        if let Some(current) = snapshot_process_names() {
            ...
            for (pid, name) in &current {
                if !known.contains(pid) {
                    ...
                    if rules.banned_process_names.iter().any(|b| b.eq_ignore_ascii_case(name)) {
...
fn network_adapter_monitor_loop() {
    unsafe { let _ = SetThreadDescription(windows::Win32::System::Threading::GetCurrentThread(), windows::core::w!("network_monitor")); }

    let mut known = match enumerate_adapter_names() {
        Some(set) => {
            logger().info("net-adapter", "baseline captured", &[("count", &set.len().to_string())]);
            set
        }
        None => {
            logger().error("net-adapter", "failed to capture baseline — monitor disabled", &[]);
            return;
        }
    };
    ...
    loop {
        ...
        let Some(current) = enumerate_adapter_names() else { continue };
        let rules = fastlane_rules().lock().unwrap().clone();

        for adapter in current.difference(&known) {
            let suspect = rules.tether_adapter_hints.iter().any(|h| adapter.to_lowercase().contains(&h.to_lowercase()));
            append_event(&serde_json::json!({
    ...
    unsafe {
        ...
        if !authorized {
            logger().warn(
                ...
                &[("client_pid", &client_pid.to_string()), ("expected_pid", &expected_pid.to_string())],
            );
        ...
        let response = match IpcRequest::parse_line(&line) {
            ...
            Ok(IpcRequest::Freeze { duration_secs, reason }) => {
...
fn ipc_server_loop() {
    unsafe { let _ = SetThreadDescription(windows::Win32::System::Threading::GetCurrentThread(), windows::core::w!("ipc_server")); }
...
fn spawn_brain() {
    ...
    use windows::Win32::Storage::FileSystem::{CreateFileW, FILE_SHARE_READ, FILE_SHARE_WRITE, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, FILE_GENERIC_WRITE};

    let log_path = wide("C:\\ProgramData\\Obylon\\logs\\brain_stdout.log");
    ...
    unsafe {
        ...
        if ok.is_err() {
            logger().error("brain", "failed to spawn Python brain", &[("exe", &exe), ("error", &format!("{:?}", GetLastError()))]);
            return;
        }
        logger().info("brain", "brain process spawned", &[("pid", &proc_info.dwProcessId.to_string())]);
