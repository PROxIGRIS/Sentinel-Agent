...
func (v *Vault) Save() error {
	...
	_ = platform.UnhideFile(v.path) // best-effort; fine if the file doesn't exist yet
	if writeErr := os.WriteFile(v.path, enc, 0o600); writeErr != nil {
...
func (v *Vault) writeFallback(enc []byte, causeErr error) error {
	...
	if werr := os.WriteFile(fallback, enc, 0o600); werr != nil {
